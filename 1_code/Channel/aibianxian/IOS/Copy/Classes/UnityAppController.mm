...
extern "C" {
    ...
    const char* PushDeviceToken() {
        if([gStrPushDeviceToken isEqual:[NSNull null]])
            return NULL;

        const char *raw = [gStrPushDeviceToken UTF8String];
        char *mem = (char*)malloc(strlen(raw) + 1);
        strcpy(mem, raw);
...
- (void)application:(UIApplication*)application didReceiveRemoteNotification:(NSDictionary*)userInfo fetchCompletionHandler:(nonnull void (^)(UIBackgroundFetchResult))completionHandler
{
    /*if(application.applicationState == UIApplicationStateActive) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[[[userInfo objectForKey:@"aps"] objectForKey:@"alert"] objectForKey:@"title"] message:[[[userInfo objectForKey:@"aps"] objectForKey:@"alert"] objectForKey:@"body"] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        return;
    }
...
- (void)application:(UIApplication*)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData*)deviceToken
{
    ...
    gStrPushDeviceToken = token;
    
    NSLog(@"%@",gStrPushDeviceToken);
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    [UMConfigure initWithAppkey:@"5e985d12570df3f1bf000073" channel:@"App Store"];
