...
public class UniWebView: MonoBehaviour {
    ...
    Rect NextFrameRect() {
        if (referenceRectTransform == null) {
            ...
            print("<-----Set referenceRectTransform----->");
    ...
    public void AddMessage(LuaFunction eventMessageReceived)
    {
        this.OnMessageReceived += (view, message) =>
            {
                if (eventMessageReceived != null)
                    eventMessageReceived.Call(view, message);
            };
    }

    public void AddMessageErrorReceived(LuaFunction eventPageErrorReceived)
    {
        this.OnPageErrorReceived += (view, error, message) =>
            {
                if (eventPageErrorReceived != null)
                    eventPageErrorReceived.Call(view, error, message);
            };
    }

    public void AddMessageShouldClose(LuaFunction eventShouldClose)
    {
        this.OnShouldClose += (view) =>
            {
                if (eventShouldClose != null)
                    eventShouldClose.Call(view);
                return false;
            };
    }

    public void AddMessagePageFinish(LuaFunction eventPageFinish)
    {
            this.OnPageFinished  += (view, statusCode , url) =>
            {
                if (eventPageFinish != null)
                    eventPageFinish.Call(view, statusCode , url);
            };
    }    

    public void ShowToEvaluateJS()
    {
         this.EvaluateJavaScript("webviewWillAppear();", (payload) => {
                 print("ShowToEvaluateJS--->dispatchEvent----payload");
        });
    }

    public void RemoveAllMessageReceive()
    {
        Delegate[] allDelegate = this.OnMessageReceived.GetInvocationList();
        for (int i = 0; i < allDelegate.Length; i++)
        {
            this.OnMessageReceived -= allDelegate[i] as MessageReceivedDelegate;
        }
    }

    public void SetFrame()
    {
        this.Frame = new Rect(0, 0, Screen.width, Screen.height);
    }
