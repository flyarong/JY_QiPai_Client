...
DdzPDKMatchMyCardUiManger = basefunc.class()

local rule={
center="center", --居中
}
--管理规则
DdzPDKMatchMyCardUiManger.mangerRule=nil
--node
DdzPDKMatchMyCardUiManger.node=nil
--牌列表
DdzPDKMatchMyCardUiManger.itemList=nil
--组件的宽度
DdzPDKMatchMyCardUiManger.itemWidth=0
DdzPDKMatchMyCardUiManger.width=0
--item间隔
DdzPDKMatchMyCardUiManger.itemInterval=0
--
DdzPDKMatchMyCardUiManger.moveDuration=0.2
local instance
function DdzPDKMatchMyCardUiManger.Create(node,itemWidth,itemHeight,itemInterval)
	instance=DdzPDKMatchMyCardUiManger.New()
...
function DdzPDKMatchMyCardUiManger:MyExit()
...
--[[
	...
	no
--]]
function DdzPDKMatchMyCardUiManger:addItemByOrder(item,isRefresh)
...
--按权值添加item
function DdzPDKMatchMyCardUiManger:addItemByWeight(item,isRefresh)
	local pos=nil
...
--从容器中移除
function DdzPDKMatchMyCardUiManger:RemoveItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			self.cardCount=self.cardCount-1
...
--删除
function DdzPDKMatchMyCardUiManger:DeleteItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			value:Destroy()
			self.cardCount=self.cardCount-1
...
--删除all
function DdzPDKMatchMyCardUiManger:DeleteAllItem()
	for idx,value in ipairs(self.itemList) do
		value:Destroy()
	end
	self.cardCount=0
...
--计算Item位置
function DdzPDKMatchMyCardUiManger:CalculationPos()
	if self.mangerRule==rule.center then
 		self:CalculationPosByCenter()
	end
end
function DdzPDKMatchMyCardUiManger:Refresh()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
	...
	if DdzPDKMatchModel.data then
		if DdzPDKMatchModel.data.dizhu then
			if DdzPDKMatchModel.data.dizhu == DdzPDKMatchModel.data.seat_num then
				if self.itemList and #self.itemList>0 then
					PDKDdzCardTag.New(self.itemList[1].transform:Find("@card_img/@tag"),PDKDdzCardTagType.sp,1)
				end
			end
		end
	end
end
function DdzPDKMatchMyCardUiManger:RefreshWithAni()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzPDKMatchMyCardUiManger:CalculationPosByCenter()
	local offsetX=0
...
function DdzPDKMatchMyCardUiManger:setRect(width,height,offsetX)
...
function DdzPDKMatchMyCardUiManger:clearItemChooseStatus()
end

function DdzPDKMatchMyCardUiManger:chooseIetm(pos)
...
--智能补全
function DdzPDKMatchMyCardUiManger:intelligent_completion(list,frist_tanqi)
	local data=DdzPDKMatchModel.data
	if list and #list>0 and #list<18 and data and data.status==DdzPDKMatchModel.Status.cp and data.cur_p==data.seat_num then
		local pos=nor_pdk_base_lib.get_real_chupai_pos_by_act(data.action_list)
		local type=0
		local pai=nil
		if pos then
			DdzPDKMatchModel.ddz_algorithm:get_cpInfo_by_action(data.action_list[pos])
			type=data.action_list[pos].type
			pai=data.action_list[pos].pai
		end

		local ic_map=DdzPDKMatchModel.ddz_algorithm:intelligent_completion(type,pai,list,data.my_pai_list,data.laizi,frist_tanqi)
		...
		if ic_map then
			...
			for idx,v in ipairs(self.itemList) do
				if v.posStatus==1 then
					if v.no<60 then
						_pai=nor_pdk_base_lib.pai_map[v.no]
					else
						_pai=data.laizi
					end
...
function DdzPDKMatchMyCardUiManger:update()
