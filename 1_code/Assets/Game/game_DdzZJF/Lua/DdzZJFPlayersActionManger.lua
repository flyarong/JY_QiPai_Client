...
DdzFKPlayersActionManger = basefunc.class()
local instance
function DdzFKPlayersActionManger.Create(gamePanel)
	instance =DdzFKPlayersActionManger.New()
	--no 2 object
	...
	instance.uiManager=DdzFKMyCardUiManger.Create(instance.gamePanel.playerself_operate_son.cards_remain,212,302,90)
	instance.cardObj = GetPrefab("DdzCard")
	return instance
end
function DdzFKPlayersActionManger:UiManagerRefresh()
	local hash={}
	local flag=false
	local my_pai_list=nil
	if DdzFKModel.data then
		my_pai_list=nor_ddz_base_lib.norId_convert_to_lzId(DdzFKModel.data.my_pai_list,DdzFKModel.data.laizi)
		if my_pai_list then
...
function DdzFKPlayersActionManger:Refresh()
...
--添加牌
function DdzFKPlayersActionManger:AddPai(pai_list)
...
function DdzFKPlayersActionManger:Fapai(pai_list)
...
function DdzFKPlayersActionManger:CreateLz(playAnim)
	self:UiManagerRefresh()
	if playAnim then
		--插入动画
		for no,v in pairs(self.my_pai_hash) do
...
--加倍回调
function DdzFKPlayersActionManger:JiabeiBtnCB()
...
--不加倍回调
function DdzFKPlayersActionManger:BujiabeiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--jdz 1 回调
function DdzFKPlayersActionManger:Jdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--jdz 2 回调
function DdzFKPlayersActionManger:Jdz2BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--jdz 3 回调
function DdzFKPlayersActionManger:Jdz3BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--不叫地主
function DdzFKPlayersActionManger:Bujdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
-- 抢地主
function DdzFKPlayersActionManger:Qdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--不抢地主
function DdzFKPlayersActionManger:Buqdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
...
--出牌回调
function DdzFKPlayersActionManger:ChupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
	if m_data then
		--情况自动出最后一手牌回调
		...
		local ret, list_table = DdzFKModel.data.ddz_algorithm:check_chupai_safe(m_data.action_list, pai_list, m_data.laizi, DdzFKModel.data.remain_pai_amount[DdzFKModel.data.seat_num])
		if ret then
			...
			if index == 1 then
				...
				local gamePanel = DdzFKLogic.get_cur_panel()
				if gamePanel then
...
function DdzFKPlayersActionManger:SendChupaiRequest(act)
	local m_data=DdzFKModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	if m_data and Network.SendRequest("nor_ddz_nor_chupai", act) then
		if m_data.countdown and m_data.countdown>0 then
			act.myrate=DdzFKModel.data.my_rate
			self:DealAction(1,act)
		end
	else
		DDZAnimation.Hint(2,Vector3.New(0,-350,0), Vector3.New(0,0,0))
...
--不出牌和要不起回调
function DdzFKPlayersActionManger:ChangeCardsPosBtnCB()
	--将牌复位牌
	for no,v in pairs(self.my_pai_hash) do
		v:ChangePosStatus(0)
	end
...
--不出牌和要不起回调
function DdzFKPlayersActionManger:BuchupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzFKModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
...
--提示回调
function DdzFKPlayersActionManger:HintBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	local get_hint_by_act=function ()
		--选取
		local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(DdzFKModel.data.action_list)
		local type=nil
		local pai=nil
		if pos then
			type=DdzFKModel.data.action_list[pos].type
			pai=DdzFKModel.data.action_list[pos].pai
		end
		self.my_hint_chupai=DdzFKModel.data.ddz_algorithm:cp_hint(type,pai, DdzFKModel.data.my_pai_list,DdzFKModel.data.laizi)
	end

	if not self.my_hint_chupai or self.my_hint_chupai.type==0  then
		get_hint_by_act()
	else
		self.my_hint_chupai=DdzFKModel.data.ddz_algorithm:cp_hint(self.my_hint_chupai.type,self.my_hint_chupai.pai, DdzFKModel.data.my_pai_list,DdzFKModel.data.laizi)
	end
	--循环
	if not self.my_hint_chupai or self.my_hint_chupai.type==0 then
		get_hint_by_act()
...
function DdzFKPlayersActionManger:DealAction(seatNum,action)
	if seatNum==1 then
		if action.type<100 and action.type>0 then
			DdzFKModel.data.ddz_algorithm:get_cpInfo_by_action(action)
		end
...
function DdzFKPlayersActionManger:CheckActionIsRun(action)
	...
	--从服务器发过来的
		...
		if not self.my_last_action then
			...
			if checkActionIsEquel(action,self.my_last_action) then
				...
				--刷新我的牌列表
				self:Refresh()
				self.gamePanel.DdzFKActionUiManger:RefreshActionWithAni(1,action)
...
--出牌action
function DdzFKPlayersActionManger:ChupaiAction(seatNum,action)
	...
	--提交给actionUimanger
	self.gamePanel.DdzFKActionUiManger:RefreshActionWithAni(seatNum,action)

	--刷新warning和剩余的pai
	self.gamePanel:RefreshRemainPaiWarningStatusWithAni(seatNum,action.type,remain_card) 
end
--其他操作
function DdzFKPlayersActionManger:OtherAction(seatNum,action)
	--如果是我自己的操作
	if seatNum==1 then
...
function DdzFKPlayersActionManger:MyExit()
	if instance then
		instance.cardObj = nil
		if instance.uiManager then
...
function DdzFKPlayersActionManger:ChangeClickStatus(status)
