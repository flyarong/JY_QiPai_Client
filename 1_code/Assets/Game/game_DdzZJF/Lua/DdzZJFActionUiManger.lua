...
function DdzFKActionUiManger:RefreshAction(seatNum,act)
	...
	dump(act,"<color=red>行为0000000000</color>")
...
function DdzFKActionUiManger:RefreshActionWithAni(seatNum,act)
	...
	if act.type>99 or act.type==0 then
		local sound
		dump(act.type,"<color=red>当前的类型---------</color>")
		if act.type==0 then
		...
		elseif act.type==102 then
			if act.rate==0 then-- 不抢地主
				...
				sound="sod_game_rob"
				DdzFKModel.data.er_qiang_dizhu_count = DdzFKModel.data.er_qiang_dizhu_count + 1
				local qtext = self.playerOperateUI[1].qiang_hint_txt
				if IsEquals(qtext) then
					qtext.text = "抢" .. DdzFKModel.data.er_qiang_dizhu_count .. "次"
				end
			end	
		end
