...
function DdzZJFGamePanel:Awake()
    ...
    self.playerInfoUI[1].shop_btn.onClick:AddListener(
        function()
    ...
    self.HelpButton.onClick:AddListener(
        function()
            ...
            DdzHelpPanel.Create(type2str[DdzFKModel.data.game_type])
        end
    )
...
function DdzZJFGamePanel:OnShareClick()
    ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
    local share_cfg = basefunc.deepcopy(share_link_config.url_dzzfk)
    share_cfg.title = string.format(share_cfg.title, DdzFKModel.data.friendgame_room_no)
    share_cfg.description = string.format(share_cfg.description, self.RuleText.text)
    share_cfg.url = string.format(share_cfg.title, MainLogic.GetPTDeeplinkKeyword(), DdzFKModel.data.friendgame_room_no, MainModel.UserInfo.user_id)
...
function DdzZJFGamePanel:MyInit()
    ...
    basefunc.handler(self.DdzFKPlayersActionManger, self.DdzFKPlayersActionManger.Jdz1BtnCB)
    EventTriggerListener.Get(self.playerself_operate_son.jdz_not_er_btn.gameObject).onClick =
    basefunc.handler(self.DdzFKPlayersActionManger, self.DdzFKPlayersActionManger.Bujdz1BtnCB)
    --二人场抢地主
    EventTriggerListener.Get(self.playerself_operate_son.qdz_1_btn.gameObject).onClick =
    basefunc.handler(self.DdzFKPlayersActionManger, self.DdzFKPlayersActionManger.Qdz1BtnCB)
    EventTriggerListener.Get(self.playerself_operate_son.qdz_not_btn.gameObject).onClick =
    basefunc.handler(self.DdzFKPlayersActionManger, self.DdzFKPlayersActionManger.Buqdz1BtnCB) 
end
        ...
        if  m_data.model_status == DdzFKModel.Model_Status.wait_join or m_data.model_status == DdzFKModel.Model_Status.wait_begin  then
            ...
            if MainModel.UserInfo.user_id == m_data.room_owner  then 
                ...
                self.ready_btn.onClick:AddListener(
                    function ()
                        ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
                        if DdzFKModel.CheakCanReadyGame() then 
                            ...
                            HintPanel.Create(1,"鲸币不足，无法继续游戏",function()
                                Event.Brocast("show_gift_panel")
                            end )
                        end
                    end
                ...
                self.ready_btn.onClick:AddListener(
                    function ()
                        ExtendSoundManager.PlaySound(audio_config.game.com_but_cancel.audio_name)
                        if DdzFKModel.CheakCanReadyGame() then 
                            Network.SendRequest("zijianfang_ready",{opt = DdzFKModel.GetReadyStatusByID(MainModel.UserInfo.user_id) and 0 or 1},"",function(_data)
...
function DdzZJFGamePanel:RefreshMenu()
    if DdzFKModel.GetCurrBeiShu() then 
        self.RuleText.text = "封顶倍数:"..DdzFKModel.GetCurrBeiShu().."  "..GameZJFModel.get_ddz_enter_wanfa_by_type(DdzFKModel.data.game_type)
    end 
end
-- 刷新中间区域(方位)
function DdzZJFGamePanel:RefreshCenter()
    ...
    if data then
        ...
        if data.friendgame_room_no and data.password == 1  then
        ...
        if data.game_type then
            ...
            elseif data.game_type == "nor_ddz_lz" then
                typeStr = "癞子斗地主"
            elseif data.game_type == "nor_ddz_er" then
                typeStr = "二人斗地主"
            elseif data.game_type == "nor_ddz_boom" then
                typeStr = "炸弹斗地主"
            end
...
function DdzZJFGamePanel:RefreshPlayerReadyStatus()
    ...
    for i = 1,DdzFKModel.data.player_count do 
        ...
        if remain_pai_amount and DdzFKModel.data.seatNum then
            ...
            if not pSeatNum then
                ...
                for i=1,3 do
                    --刷新warning
                    if self.warningUI[i] then
                        if remain_pai_amount[DdzFKModel.data.seatNum[i]] and remain_pai_amount[DdzFKModel.data.seatNum[i]]<3 then
                            --显示
                    ...
                    if self.cardsRemainUI[i] then         
                        ...
                        if DdzFKModel.data.game_type == DdzFKModel.game_type.er then
                            self.playerOperateUI[3].cards_remain.transform.gameObject:SetActive(false)
                        end
...
function DdzZJFGamePanel:RefreshEasyChat()
    ...
    for i = 1, 3 do
        if i <= DdzFKModel.maxPlayerNumber then
    ...
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        ...
        local RefreshPlayerAllInfo=function (pSeatNum)
            ...
            if info and info.base then
                ...
                VIPManager.set_vip_text(player.head_vip_txt,info.base.vip_level)
	   ...
	   if dizhu ~= nil and dizhu > 0 then
            self:ShowOrHidePlayerInfo(true,pSeatNum)
            for i = 1, DdzFKModel.maxPlayerNumber do
                local info = playerInfo[DdzFKModel.data.seatNum[i]]
                if info then
                    --地主
...
function DdzZJFGamePanel:SetScore(ScoreText, score)
    debug.traceback()
    print("<color=red>RRRRRRRRRRRRRRRRRRRRRRRRRRRR</color>")
    if score >= 0 then
...
function DdzZJFGamePanel:RefreshPermitStatus()
    ...
    if DdzFKModel.data and DdzFKModel.data.seat_num then
        ...
        if (status==DdzFKModel.Status.jdz or status==DdzFKModel.Status.jiabei or status==DdzFKModel.Status.cp or
            status == DdzFKModel.Status.q_dizhu ) and cur_p then
                ...
                if cur_p==data.seat_num then
                    ...
                    if permitData then
                        ...
                        if permitData.type==DdzFKModel.Status.jdz then
                            ...
                            self.playerself_operate_son.jiaodizhu.gameObject:SetActive(false)
                        ...
                        elseif  permitData.type==DdzFKModel.Status.jiabei then
                        ...
                        elseif permitData.type == DdzFKModel.Status.q_dizhu then
...
function DdzZJFGamePanel:MakeLister()
    ...
    lister["DDZ_ResetAutoExitTime"] = basefunc.handler(self, self.ResetAutoExitTime)
    lister["model_score_change"] = basefunc.handler(self, self.model_score_change)
end
...
--刷新废牌
function DdzZJFGamePanel:RefreshDeadwood()
    if DdzFKModel.data.game_type ~= "nor_ddz_er" then
        return
    end
    print("<color=red>RefreshDeadwood>>>>>>>>>>>>>>>>>>>>>>>>> </color>")
    self:DelDeadwood()
    local mData = DdzFKModel.data
    if mData.status == DdzFKModel.Status.fp or mData.status == DdzFKModel.Status.settlement or mData.deadwood_list then
...
--二人
function DdzZJFGamePanel:DelDeadwood()
...
--抢地主
function DdzZJFGamePanel:RefreshQDZUI()
    if DdzFKModel.data.game_type ~= DdzFKModel.game_type.er then
        return
    end
    local m_data = DdzFKModel.data
    if
        m_data and m_data.status == DdzFKModel.Status.q_dizhu and m_data.er_qiang_dizhu_count and
...
-- 刷新让牌提示
function DdzZJFGamePanel:RefreshRangHint()
    if DdzFKModel.data.game_type ~= DdzFKModel.game_type.er then
        return
    end
    local m_data = DdzFKModel.data
    if m_data and m_data.dizhu and m_data.remain_pai_amount then
        ...
        local num = m_data.remain_pai_amount[DdzFKModel.GetSeatNM()] - m_data.rangpai_num
        if num < 0 then
            num = 0
        end
...
function DdzZJFGamePanel:ResetAutoExitTime(time)
    self.aoto_exit_time = time or 30
    LittleTips.Create("准备倒计时已重置，请尽快准备")
end

function DdzZJFGamePanel:model_score_change(data)
    local cSeat = DdzFKModel.data.s2cSeatNum[data.seat_num]
    local playerUI = self.playerInfoUI[cSeat]
    self:SetScore(playerUI.score2_txt, DdzFKModel.data.playerInfo[data.seat_num].base.score)
    dump(data.change,"<color=red>uuuuuuuuuuuuuuuu</color>")
    DDZAnimation.ChangeScore(cSeat, data.change ,playerUI.score_change_pos)
end