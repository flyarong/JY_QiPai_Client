...
function DdzFKClearing:ctor(parent)
	...
	self.leftNode = tran:Find("Genter/Left")
...
function DdzFKClearing:InitRect()
	...
	if DdzFKModel.data.game_type == DdzFKModel.game_type.er then
		self.BaseRateText.text = settlement_data.q_dizhu == 0 and "--" or "x" .. settlement_data.q_dizhu
		self.leftNode.gameObject:SetActive(false)
	else
	...
	for i=1,DdzFKModel.maxPlayerNumber do
		...
		self.playerInfo[i].CurScore.text = sData.award[pSeat] >= 0 and "+"..StringHelper.ToCash(sData.award[pSeat] ) or "-"..StringHelper.ToCash(sData.award[pSeat] * -1)
		...
		self.playerInfo[i].AllScore.text = pData[pSeat].base.score >= 0 and StringHelper.ToCash(pData[pSeat].base.score) or pData[pSeat].base.score
	end
	self:UpdateGameOver()
