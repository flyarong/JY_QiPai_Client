...
function C:ctor(data,yes_callback,no_callback)
	...
	self.person = DdzFKModel.data.game_type == "nor_ddz_er" and 2 or 3
	--总的进入条件（房主看到的和非房主看到的不一致）
	if DdzFKModel.IsFZPaY() then
		xishu = 0
	else
		xishu = GameZJFModel.get_ddz_enter_xishu_by_type(DdzFKModel.data.game_type)
	end
	enter_base = GameZJFModel.get_ddz_enter_base_by_type(DdzFKModel.data.game_type)
...
function C:InitUI()
	...
	self.old_jrtj_txt.text = (DdzFKModel.get_ori_game_cfg_byOption("enter_limit") *  DdzFKModel.GetCurrBeiShu() + xishu ) * DdzFKModel.get_ori_game_cfg_byOption("init_stake") + enter_base
	...
	self.name_txt.text = "房主【"..DdzFKModel.GetFzName().."】修改了房间规则"
