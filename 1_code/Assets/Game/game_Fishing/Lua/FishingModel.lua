...
function FishingModel.UseSkill(type)
    ...
    if type == "prop_fish_lock" then
        if userdata.lock_state == "nor" then
            if userdata.base.score >= FishingModel.GetSkillMoney("prop_fish_lock") or userdata.prop_fish_lock > 0  then
    ...
    if type == "prop_fish_frozen" then
        ...
        local userdata = FishingModel.GetPlayerData()
        if userdata.frozen_state == "nor" then
            if userdata.base.score >= FishingModel.GetSkillMoney("prop_fish_frozen") or userdata.prop_fish_frozen > 0 then
...
function FishingModel.GetCanEnterID()
    ...
    local check_func = function (game_id)
    ...
    for k,v in ipairs(this.Config.fish_hall_list) do
        if (not v.enter_min or v.enter_min <= all_score) and (not v.enter_max or v.enter_max >= all_score) and check_func(v.game_id) then
            return v.game_id
        end
    end
end
