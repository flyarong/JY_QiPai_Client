...
function C:SendShoot(vec, auto, FishID)
    ...
    local all_score = FishingMatchModel.data.score + FishingMatchModel.data.wait_add_score
    local cur_gun_rate = FishingMatchModel.GetGunCfg(user.gun_info.show_bullet_index, user.seat_num).gun_rate
    ...
    if not FishingActivityManager.CheckHaveBullet(self.seat_num) then
        if bullet_num <= 0 and all_score < cur_gun_rate then
            if user.isPC and (FishID or self.mControlMode == ControlMode.Auto) then
                return
            end
            isPC = true
            user.isPC = true
            Event.Brocast("model_player_PC", user.seat_num)
            return
        end
