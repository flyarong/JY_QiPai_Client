...
local instance
DdzPDKPlayersActionManger = basefunc.class()
function DdzPDKPlayersActionManger.Create(gamePanel)
	instance=DdzPDKPlayersActionManger.New()
	--no 2 object
	...
	instance.uiManager=DdzPDKMyCardUiManger.Create(instance.gamePanel.playerself_operate_son.cards_remain,212,302,90)
	instance.cardObj = GetPrefab("DdzCard")
	return instance
end

function DdzPDKPlayersActionManger:UiManagerClearAndRefresh()
...
function DdzPDKPlayersActionManger:UiManagerRefresh()
	local hash={}
	local flag=false
	local my_pai_list=nil
	if DdzPDKModel.data then
		my_pai_list=nor_ddz_base_lib.norId_convert_to_lzId(DdzPDKModel.data.my_pai_list,DdzPDKModel.data.laizi)
		if my_pai_list then
...
function DdzPDKPlayersActionManger:Refresh()
...
--添加牌
function DdzPDKPlayersActionManger:AddPai(pai_list)
...
function DdzPDKPlayersActionManger:Fapai(pai_list)
	self.uiManager:DeleteAllItem()
	local aniPaiList = {}
	for k,v in ipairs(pai_list) do
		local card=DdzCard.New(self.cardObj, self.uiManager.node,v,v,0)
...
function DdzPDKPlayersActionManger:CreateLz(playAnim)
	self:UiManagerRefresh()
	if playAnim then
		--插入动画
		for no,v in pairs(self.my_pai_hash) do
...
--加倍回调
function DdzPDKPlayersActionManger:JiabeiBtnCB()
...
--不加倍回调
function DdzPDKPlayersActionManger:BujiabeiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--jdz 1 回调
function DdzPDKPlayersActionManger:Jdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--jdz 2 回调
function DdzPDKPlayersActionManger:Jdz2BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--jdz 3 回调
function DdzPDKPlayersActionManger:Jdz3BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--不叫地主
function DdzPDKPlayersActionManger:Bujdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
-- 抢地主
function DdzPDKPlayersActionManger:Qdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--不抢地主
function DdzPDKPlayersActionManger:Buqdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--闷拉倒 start
--闷抓 回调
function DdzPDKPlayersActionManger:MenZhuaBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--看牌 回调
function DdzPDKPlayersActionManger:KanPaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--抓牌 回调
function DdzPDKPlayersActionManger:ZhuaPaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--不抓牌 回调
function DdzPDKPlayersActionManger:BuZhuaPaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--倒 回调
function DdzPDKPlayersActionManger:DaoBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--不倒
function DdzPDKPlayersActionManger:BuDaoBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
...
--拉
function DdzPDKPlayersActionManger:LaBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
	if m_data and Network.SendRequest("nor_ddz_mld_dao_la", {opt=1}) then
		if m_data.countdown and m_data.countdown>0 then
...
--不拉
function DdzPDKPlayersActionManger:BuLaBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
	if m_data and Network.SendRequest("nor_ddz_mld_dao_la", {opt=0}) then
		if m_data.countdown and m_data.countdown>0 then
...
--出牌回调
function DdzPDKPlayersActionManger:ChupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
	if m_data then
		--情况自动出最后一手牌回调
		...
		local ret, list_table = DdzPDKModel.ddz_algorithm:check_chupai_safe(m_data.action_list, pai_list, m_data.laizi, DdzPDKModel.data.remain_pai_amount[DdzPDKModel.data.seat_num])
		if ret then
			...
			if true or index == 1 then
				...
				local gamePanel = DdzPDKLogic.get_cur_panel()
				if gamePanel then
...
function DdzPDKPlayersActionManger:SendChupaiRequest(act)
	local m_data=DdzPDKModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	if m_data and Network.SendRequest("nor_pdk_nor_chupai", act) then
		if m_data.countdown and m_data.countdown>0 then
			act.myrate=DdzPDKModel.data.my_rate
			self:DealAction(1,act)
		end
	else
		DDZAnimation.Hint(2,Vector3.New(0,-350,0), Vector3.New(0,0,0))
...
--不出牌和要不起回调
function DdzPDKPlayersActionManger:ChangeCardsPosBtnCB()
	--将牌复位牌
	for no,v in pairs(self.my_pai_hash) do
		v:ChangePosStatus(0)
	end
...
--不出牌和要不起回调
function DdzPDKPlayersActionManger:BuchupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzPDKModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
...
--提示回调
function DdzPDKPlayersActionManger:HintBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(DdzPDKModel.data.action_list)
	local type
	local pai
	if pos then
		type=DdzPDKModel.data.action_list[pos].type
		pai=DdzPDKModel.data.action_list[pos].pai	
	end
	...
	local get_hint_by_act=function ()
		...
		if pos then
			if remain_pai_amount[xj_seat] == 1 and type == 1 then
				self.my_hint_chupai = DdzPDKModel.ddz_algorithm:cp_hint_baopei(type,pai, DdzPDKModel.data.my_pai_list)
			else
				self.my_hint_chupai=DdzPDKModel.ddz_algorithm:cp_hint(type,pai, DdzPDKModel.data.my_pai_list,DdzPDKModel.data.laizi)
			end
		else
			local result=DdzPDKModel.ddz_algorithm:cp_hint(type,pai, DdzPDKModel.data.my_pai_list,DdzPDKModel.data.laizi)		
	...
	-- 	get_hint_by_act()
	-- else
	-- 	self.my_hint_chupai=DdzPDKModel.ddz_algorithm:cp_hint(self.my_hint_chupai.type,self.my_hint_chupai.pai, DdzPDKModel.data.my_pai_list,DdzPDKModel.data.laizi)
	-- end
	-- --循环
	-- if not self.my_hint_chupai or self.my_hint_chupai.type==0 then
	-- 	get_hint_by_act()
    ...
    if pos then
    	if not self.my_hint_chupai  or self.my_hint_chupai.type==0 then
	    	...
	    	if remain_pai_amount[xj_seat] == 1 and type == 1 then
				self.my_hint_chupai=DdzPDKModel.ddz_algorithm:cp_hint_baopei(type,pai, DdzPDKModel.data.my_pai_list,self.my_hint_chupai.type,self.my_hint_chupai.pai)
			else
				self.my_hint_chupai=DdzPDKModel.ddz_algorithm:cp_hint(self.my_hint_chupai.type,self.my_hint_chupai.pai, DdzPDKModel.data.my_pai_list,DdzPDKModel.data.laizi)
			end
	    end
...
function DdzPDKPlayersActionManger:DealAction(seatNum,action)
	if seatNum==1 then
		self.all_pai = nil
		if action.type<100 and action.type>0 then
			DdzPDKModel.ddz_algorithm:get_cpInfo_by_action(action)
		end
...
function DdzPDKPlayersActionManger:CheckActionIsRun(action)
	...
	--从服务器发过来的
		...
		if not self.my_last_action then
			...
			if checkActionIsEquel(action,self.my_last_action) then
				...
				--刷新我的牌列表
				self:Refresh()
				self.gamePanel.DdzPDKActionUiManger:RefreshActionWithAni(1,action)
...
--出牌action
function DdzPDKPlayersActionManger:ChupaiAction(seatNum,action)
	...
	--提交给actionUimanger
	self.gamePanel.DdzPDKActionUiManger:RefreshActionWithAni(seatNum,action)

	--刷新warning和剩余的pai
	self.gamePanel:RefreshRemainPaiWarningStatusWithAni(seatNum,action.type,remain_card) 
end
--其他操作
function DdzPDKPlayersActionManger:OtherAction(seatNum,action)
	--如果是我自己的操作
	if seatNum==1 then
...
function DdzPDKPlayersActionManger:MyExit()
	if instance then
...
function DdzPDKPlayersActionManger:ChangeClickStatus(status)
