...
DdzPDKMyCardUiManger = basefunc.class()

local rule={
center="center", --居中
}
--管理规则
DdzPDKMyCardUiManger.mangerRule=nil
--node
DdzPDKMyCardUiManger.node=nil
--牌列表
DdzPDKMyCardUiManger.itemList=nil
--组件的宽度
DdzPDKMyCardUiManger.itemWidth=0
DdzPDKMyCardUiManger.width=0
--item间隔
DdzPDKMyCardUiManger.itemInterval=0
--
DdzPDKMyCardUiManger.moveDuration=0.2
local instance
function DdzPDKMyCardUiManger.Create(node,itemWidth,itemHeight,itemInterval)
	instance=DdzPDKMyCardUiManger.New()
...
function DdzPDKMyCardUiManger:MyExit()
...
--[[
	...
	no
--]]
function DdzPDKMyCardUiManger:addItemByOrder(item,isRefresh)
...
--按权值添加item
function DdzPDKMyCardUiManger:addItemByWeight(item,isRefresh)
	local pos=nil
...
--从容器中移除
function DdzPDKMyCardUiManger:RemoveItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			self.cardCount=self.cardCount-1
...
--删除
function DdzPDKMyCardUiManger:DeleteItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			value:Destroy()
			self.cardCount=self.cardCount-1
...
--删除all
function DdzPDKMyCardUiManger:DeleteAllItem()
	for idx,value in ipairs(self.itemList) do
		value:Destroy()
	end
	self.cardCount=0
...
--计算Item位置
function DdzPDKMyCardUiManger:CalculationPos()
	if self.mangerRule==rule.center then
 		self:CalculationPosByCenter()
	end
end
function DdzPDKMyCardUiManger:Refresh()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzPDKMyCardUiManger:RefreshWithAni()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzPDKMyCardUiManger:CalculationPosByCenter()
	local offsetX=0
...
function DdzPDKMyCardUiManger:setRect(width,height,offsetX)
...
function DdzPDKMyCardUiManger:clearItemChooseStatus()
end

function DdzPDKMyCardUiManger:chooseIetm(pos)
...
--智能补全
function DdzPDKMyCardUiManger:intelligent_completion(list,frist_tanqi)
	local data=DdzPDKModel.data
	if list and #list>0 and #list<18 and data and data.status==DdzPDKModel.Status.cp and data.cur_p==data.seat_num then
		local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(data.action_list)
		local type=0
		local pai=nil
		if pos then
			DdzPDKModel.ddz_algorithm:get_cpInfo_by_action(data.action_list[pos])
			type=data.action_list[pos].type
			pai=data.action_list[pos].pai
		end

		local ic_map=DdzPDKModel.ddz_algorithm:intelligent_completion(type,pai,list,data.my_pai_list,data.laizi,frist_tanqi)
...
function DdzPDKMyCardUiManger:update()
