...
function DdzPDKClearing:ctor(isdelay)
    ...
    Event.Brocast("pdkclear_created", {panelSelf=self})
end
...
function DdzPDKClearing:UpdateExchange()
    ...
    if is_game_over and DdzPDKModel.data.exchange_hongbao and DdzPDKModel.data.exchange_hongbao.is_exchanged == 0 and MainModel.UserInfo.jing_bi >= 6000 then
...
--查看牌型
function DdzPDKClearing:OnSeeCardClick()
	self.ShowCard = not self.ShowCard
    self:ShowDetailsAndPlayers(self.ShowCard)
end

function DdzPDKClearing:ShowDetailsAndPlayers(isShow)
    Event.Brocast("pdkclear_seecard", {isShow = isShow})
