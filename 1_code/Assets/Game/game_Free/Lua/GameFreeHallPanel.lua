...
function GameFreeHallPanel:ctor(parm)
	...
	btn_map["right_top"] = {self.btn_node2}
...
function GameFreeHallPanel:UpdateAssetInfo()
	...
	if IsEquals(self.GoldText) and IsEquals(self.RedPacketText) then
		self.GoldText.text =  StringHelper.ToCash(MainModel.UserInfo.jing_bi)
		--self.DiamondText.text = StringHelper.ToCash(MainModel.UserInfo.diamond)
		self.RedPacketText.text = StringHelper.ToRedNum(MainModel.GetHBValue())
		self:UpdateKS()
...
function GameFreeHallPanel:CallToggleClick(i)
	...
	if GuideLogic then
		...
		GuideLogic.CheckRunGuide("free_js_exit")
	end
    Event.Brocast("WQP_Guide_Check",{guide = 2 ,guide_step = 2})
...
function GameFreeHallPanel:CallEnter(game_id)
    ...
    check = function ()
        ...
        if ( GuideModel and GuideModel.data and GuideLogic and GuideLogic.IsFreeBattle() ) 
		or ( WQPGuideModel and WQPGuideModel.data and WQPGuideModel.IsFreeBattle()) then
            xsyd_signup()
        else
            signup()
        end
