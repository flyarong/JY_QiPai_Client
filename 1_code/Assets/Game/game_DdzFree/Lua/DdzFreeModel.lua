...
local function MakeLister()
    ...
    lister["nor_ddz_nor_jiabei_response"] = this.on_nor_ddz_nor_jiabei_response
...
function DdzFreeModel.on_nor_ddz_nor_jiabei_response(proto_name, data)
    dump(data, "<color=red>on_nor_ddz_nor_jiabei_response</color>")
    if data.result == 0 then
        --Event.Brocast("model_nor_ddz_nor_jiabei_response")
    else
        HintPanel.Create(1,data.result)
    end
end
--***********************DDZFree
--所有数据
function DdzFreeModel.on_fg_all_info(proto_name, data)
    ...
    if m_data then
        ...
        if is_true and m_data.model_status == DdzFreeModel.Model_Status.gameover then
...
function DdzFreeModel.on_fg_activity_data_msg(proto_name, data)
    ...
    if m_data and m_data.activity_data then
        ...
        if m_ad.activity_id == ActivityType.TianJiangCaiShen then
            ...
            Event.Brocast("activity_fg_activity_data_msg", {activity_data = m_data.activity_data,game_type = DdzFreeModel.baseData.game_type, game_id = DdzFreeModel.baseData.game_id})
        end
    end
end
...
function DdzFreeModel.on_nor_ddz_nor_action_msg(proto_name, data)
    ...
    --加倍
    if act_type == 101 and data.action.rate ~=0 then
        m_data.my_rate = m_data.my_rate * data.action.rate
        Event.Brocast("model_nor_ddz_nor_jiabei_response")
    end
