...
function DdzFreeGamePanel:Awake()
    ...
    self.cur_multiple_txt.transform:SetParent(self.ddz_match_dizhu_card_ui)
    self.cur_multiple_txt.transform.localPosition = Vector3.New(222, 528, 0)
end
...
function DdzFreeGamePanel:MyInit()
    ...
    EventTriggerListener.Get(self.playerself_operate_son.jiabei_cj_btn.gameObject).onClick =
        basefunc.handler(self.DdzFreePlayersActionManger, self.DdzFreePlayersActionManger.JiabeiCjBtnCB)
...
function DdzFreeGamePanel:MakeLister()
    ...
    lister["model_nor_ddz_nor_jiabei_response"] = basefunc.handler(self,self.nor_ddz_nor_jiabei_response)
    --资产改变
    lister["model_AssetChange"] = basefunc.handler(self, self.AssetChange)

    lister["ddzfreeclear_created"] = basefunc.handler(self, self.on_ddzfreeclear_created)
...
function DdzFreeGamePanel:nor_ddz_nor_jiabei_response()
    dump("<color=red>加倍成功</color>")
...
function DdzFreeGamePanel:nor_ddz_nor_dizhu_msg()
    dump(DdzFreeModel.baseData, "<color=red>+++++nor_ddz_nor_dizhu_msg+++baseData++</color>")
    dump(DdzFreeModel.jdz_type, "<color=red>+++++nor_ddz_nor_dizhu_msg+++DdzFreeModel.jdz_type++</color>")
    if DdzFreeModel.data then
...
function DdzFreeGamePanel:on_ddzfreeclear_created()
    self:ChangeBtnNodeView(false)
end

function DdzFreeGamePanel:on_fg_close_clearing()
    local m_data = DdzFreeModel.data
    if m_data.model_status ~= DdzFreeModel.Model_Status.wait_table then
        self:ChangeBtnNodeView(true)
    end
...
function DdzFreeGamePanel:RefreshJBStatus(permitData)
    ...
    self:RefreshCjjbCardStatus()
    self:RefreshClockPos(self.playerself_operate_son.jiabei_time_pos)
end

function DdzFreeGamePanel:RefreshCjjbCardStatus()
    self.playerself_operate_son.cjjbk_num.gameObject:SetActive(true)
    local cjjbk_num = GameItemModel.GetItemCount("prop_super_jiabei_card") or 0
    dump(cjjbk_num,"<color=red>超级加倍卡的数量</color>")
    if cjjbk_num < 1 then
        self.playerself_operate_son.cjjbk_num.gameObject:SetActive(false)
    elseif cjjbk_num < 100 then
        self.playerself_operate_son.cjjbk_num_txt.text = cjjbk_num
    else
        self.playerself_operate_son.cjjbk_num_txt.text = "99+"
    end
end
...
--日常任务
function DdzFreeGamePanel:ShowDailyTask()
    dump("<color=white><-----ShowDailyTask-----></color>")
    self.dailyTaskPanel = GameManager.GotoUI({gotoui = "sys_rcrw",goto_scene_parm = "panel"})
    if self.dailyTaskPanel then
        dump("<color=white><-----SetParent-----></color>")
        self.dailyTaskPanel.transform:SetParent(self.dizhu_card_son.ddzcardc.transform)
        self.dailyTaskPanel.transform:SetSiblingIndex(0)
    end
end
...
function DdzFreeGamePanel:ChangeBtnNodeView(isShow)
    self.right_top_rect.gameObject:SetActive(isShow)
    self.left_top_node.gameObject:SetActive(isShow)
    self.left_node.gameObject:SetActive(isShow)
    self.right_btn_1.gameObject:SetActive(isShow)
end