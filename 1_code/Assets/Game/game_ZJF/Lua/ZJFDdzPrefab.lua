...
function C:ctor(parent,type_index)
	...
	self:AddMsgListener()
	self:InitUI()
	self.person = type_index == 3 and 2 or 3
	xishu = GameZJFModel.get_ddz_enter_xishu_by_type(types[type_index])
	enter_base = GameZJFModel.get_ddz_enter_base_by_type(types[type_index])
	...
	self.wf_txt.text = GameZJFModel.get_ddz_enter_wanfa_by_type(types[type_index])
	for i = 1 ,4 do
...
function C:InitUI()
	self.create_btn.onClick:AddListener(
		function ()
			if MainModel.UserInfo.jing_bi >= self:GetJRTJText() and MainModel.UserInfo.jing_bi >= self:GetDiFen() * GameZJFModel.get_ddz_create_xishu_by_type(types[self.type_index]) then 
...
function C:RefreshText()
	self.df_txt.text = StringHelper.ToCash(self:GetDiFen())
	self.jrtj_txt.text = StringHelper.ToCash(self:GetJRTJText())
