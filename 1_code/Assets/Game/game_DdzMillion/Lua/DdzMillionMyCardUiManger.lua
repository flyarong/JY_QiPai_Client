...
DdzMillionMyCardUiManger = basefunc.class()

local rule={
center="center", --居中
}
--管理规则
DdzMillionMyCardUiManger.mangerRule=nil
--node
DdzMillionMyCardUiManger.node=nil
--牌列表
DdzMillionMyCardUiManger.itemList=nil
--组件的宽度
DdzMillionMyCardUiManger.itemWidth=0
DdzMillionMyCardUiManger.width=0
--item间隔
DdzMillionMyCardUiManger.itemInterval=0
--
DdzMillionMyCardUiManger.moveDuration=0.2

local instance
function DdzMillionMyCardUiManger.Create(node,itemWidth,itemHeight,itemInterval)
	instance=DdzMillionMyCardUiManger.New()
...
function DdzMillionMyCardUiManger:MyExit()
...
--[[
	...
	no
--]]
function DdzMillionMyCardUiManger:addItemByOrder(item,isRefresh)
...
--按权值添加item
function DdzMillionMyCardUiManger:addItemByWeight(item,isRefresh)
	local pos=nil
...
--从容器中移除
function DdzMillionMyCardUiManger:RemoveItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			self.cardCount=self.cardCount-1
...
--删除
function DdzMillionMyCardUiManger:DeleteItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			value:Destroy()
			self.cardCount=self.cardCount-1
...
--删除all
function DdzMillionMyCardUiManger:DeleteAllItem()
	for idx,value in ipairs(self.itemList) do
		value:Destroy()
	end
	self.cardCount=0
...
--计算Item位置
function DdzMillionMyCardUiManger:CalculationPos()
	if self.mangerRule==rule.center then
 		self:CalculationPosByCenter()
	end
end
function DdzMillionMyCardUiManger:Refresh()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
	...
	if DdzMillionModel.data then
		if DdzMillionModel.data.dizhu then
...
function DdzMillionMyCardUiManger:RefreshWithAni()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzMillionMyCardUiManger:CalculationPosByCenter()
	local offsetX=0
...
function DdzMillionMyCardUiManger:setRect(width,height,offsetX)
...
function DdzMillionMyCardUiManger:clearItemChooseStatus()
end

function DdzMillionMyCardUiManger:chooseIetm(pos)
...
--智能补全
function DdzMillionMyCardUiManger:intelligent_completion(list)
	local data=DdzMillionModel.data
	if list and #list>0 and #list<10 and data and data.status==million_status.cp and data.cur_p==data.seat_num then
		local ic_map=nDdzFunc.intelligent_completion(data.action_list,list,data.my_pai_list)
		if ic_map then
			for idx,v in ipairs(self.itemList) do 
				if ic_map[v.no] then
					v:ChangePosStatus(1)
...
function DdzMillionMyCardUiManger:update()
