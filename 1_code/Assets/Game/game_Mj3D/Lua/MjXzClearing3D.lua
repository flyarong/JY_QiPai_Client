...
function MjXzClearing:ctor(parent)
	...
	self.ReadyButton = tran:Find("@anniu_node/ReadyButton"):GetComponent("Button")
	...
	for i = 1, 4 do
		...
		tab.HeadVipTxt = rect:Find("HeadVipTxt"):GetComponent("Text")
		tab.NameTxt = rect:Find("NameTxt"):GetComponent("Text")
		-- tab.CoinCount = rect:Find("CoinCount"):GetComponent("Text")
		tab.DetailBtn = rect:Find("Detail/DetailBtn"):GetComponent("Button")
		tab.MoneyTxt = rect:Find("MoneyTxt"):GetComponent("Text")
		tab.piaoIcon = rect:Find("piaoIcon"):GetComponent("Image")

		EventTriggerListener.Get(tab.DetailBtn.gameObject).onDown = function ()
            ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
            --self.detail_content.gameObject:SetActive(true)

			if not self.detailShowing then
				self.DetailScroll.gameObject:SetActive(true)
				self.detailShowing = true
			else
				self.DetailScroll.gameObject:SetActive(false)
				self.detailShowing = false
			end
        end
        EventTriggerListener.Get(tab.DetailBtn.gameObject).onUp = function ()
            --self.detail_content.gameObject:SetActive(false)
        end
	end
	...
	self.ShareBtn.gameObject:SetActive(GameGlobalOnOff.ShowOff)
    ...
    local btn_map = {}
	btn_map["top_right"] = { self.tr_btn_1, self.tr_btn_2 }
	btn_map["down"] = { self.djhs_node}
	btn_map["left_center"] = {self.lc_btn_1, self.lc_btn_2, self.lc_btn_3}
	btn_map["right_center"] = {self.rc_btn_1, self.rc_btn_2, self.rc_btn_3}
	...
	self.MiniGameGuide.transform:Find("Text"):GetComponent("Text").text = "发现大量福卡,立即去赚"
	Event.Brocast("mjfreeclear_created",{panelSelf = self})
end
...
function MjXzClearing:InitRect()
	...
	for _, data in ipairs(clearData) do
		...
		-- 	i = 3
		-- end
		self:InitPlayers(i, data)

		if i == 1 then
			self:InitMyPlayerInfo(data)
		end
		-- if i == 1 then
    ...
    -- if a and b then
    --     self.GameExitTimeText.transform.localPosition = Vector3.New(-225,244,0)
	-- 	self.fengding.transform.localPosition = Vector3.New(130,246,0)
...
function MjXzClearing:UpdateExchange()
    ...
    if is_game_over and MjXzModel.data.exchange_hongbao and MjXzModel.data.exchange_hongbao.is_exchanged == 0 and MainModel.UserInfo.jing_bi >= 6000 then
...
function MjXzClearing:InitPlayers(i, data)
	dump(i, "<color=white>i</color>")
	dump(data, "<color=white>SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS</color>")
	...
	VIPManager.set_vip_text(rect.HeadVipTxt,base.vip_level)
	rect.NameTxt.text = base.name
	...
	if score > 0 then
		rect.MoneyTxt.text = "+" .. StringHelper.ToCash(score)
	elseif score < 0 then
		rect.MoneyTxt.text = "-" .. StringHelper.ToCash(score)
	elseif score == 0 then
		rect.MoneyTxt.text = "+0"
	end
	--rect.CoinCount.text = StringHelper.ToCash(self:GetPlayerScore(seatno))

	if i ~= 1 then
		rect.DetailBtn.gameObject:SetActive(false)
	end
end

function MjXzClearing:InitMyPlayerInfo(data)
	local seatno = data.seat_num
	if MjXzModel.GetRealPlayerSeat () == seatno then
		...
		local a,b = GameButtonManager.RunFun({gotoui="sys_act_operator",game_id = MjXzModel.data.game_id}, "CheckShowLS")
...
function MjXzClearing:ShowDetailsAndPlayers(isShow)
	...
	self.GameExitTimeText.gameObject:SetActive(isShow)
	self.fengding.gameObject:SetActive(isShow)
...
function MjXzClearing:IsCanShare()
	if GameGlobalOnOff.ShowOff and self.my_score > 0 and (self.pai_type or self.hu_type) and MainModel.UserInfo.xsyd_status == 1 then
		return true
	end
	return false
end
