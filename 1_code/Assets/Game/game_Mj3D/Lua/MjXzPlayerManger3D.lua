...
function MjXzPlayerManger:Refresh()
	dump(self.playerData , "<color=yellow>----------------- MjXzPlayerManger:Refresh </color>")
	self:InitUI()
    ...
    if model.model_status == MjXzModel.Model_Status.wait_begin then
	...
	local playerInfo = MjXzModel.GetPosToPlayer(self.uiPos)
...
-- 分数修改
function MjXzPlayerManger:RefreshMoney()
...
function MjXzPlayerManger:MopaiPermit(pai)
	...
	if self:IsMyself() then
		if not self:checkMjMoPaiNum( self.playerData.spList ) or not self:checkMjMoPaiNum( self.ShouPai.spList ) then
			...
			MjXzLogic.on_mjfg_status_error_msg()
		end
	end
...
function MjXzPlayerManger:ShowCards(cardList, huPai)
	if not self:IsMyself() and cardList and #cardList > 0 then
		self.ShouPai:ShowCards(cardList, huPai)
	end
end

function MjXzPlayerManger:ClearShowCards()
	if not self:IsMyself() then
