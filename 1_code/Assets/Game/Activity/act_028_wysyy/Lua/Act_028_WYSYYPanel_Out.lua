...
Act_028_WYSYYPanel_Out = basefunc.class()
local C = Act_028_WYSYYPanel_Out
C.name = "Act_028_WYSYYPanel_Out"
...
function C:ctor(parent)
	...
	self.fhsmp_btn.onClick:AddListener(function()
		GameManager.GotoUI( {gotoui =  "act_066_fhsmp", goto_scene_parm = "panel"} )
	end)

	local a,b = GameButtonManager.RunFun({gotoui = "act_066_fhsmp"}, "IsCanBuyShop")
	if a then
		self.fhsmp_btn.gameObject:SetActive(b)
		-- self.huxi = CommonHuxiAnim.Go(self.fhsmp_btn.gameObject)
		-- self.huxi.Start()
	end
end
