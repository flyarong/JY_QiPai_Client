...
Act_020_XGSJBManager = {}
local M = Act_020_XGSJBManager
M.key = "act_020_xgsjb"
GameButtonManager.ExtLoadLua(M.key, "Act_020_XGSJBPanel")
local this
local lister
...
function M.IsActive()
    ...
    -- 对应权限的key
    local _permission_key = "actp_rank_xiguanzouji_20_7_rank"
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_020_XGSJBPanel.Create(parm.parent)
        end
    end 
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_020_XGSJBPrefab",btn_gameObject.transform)
...
function M.query_rank_base_info_response(_,data)
    dump(data,"<color=red><size=30>西瓜鱼排行榜数据------</size></color>")
