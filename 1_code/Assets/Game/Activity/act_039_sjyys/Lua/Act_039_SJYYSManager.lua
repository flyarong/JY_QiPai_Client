...
Act_039_SJYYSManager = {}
local M = Act_039_SJYYSManager
M.key = "act_039_sjyys"
GameButtonManager.ExtLoadLua(M.key, "Act_039_SJYYSPanel") 
GameButtonManager.ExtLoadLua(M.key, "Act_039_SJYYSPanel_Out") 
local lister
local m_data
...
local end_time = 1606568400  --
local start_time = 1606174200   --
local match_start_time = 1606568400 
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_039_SJYYSPanel.Create(parm.parent)
    elseif parm.goto_scene_parm == "panel_out" then
        return Act_039_SJYYSPanel_Out.Create(parm.parent)
...
function M.on_act_match_order_msg_change(data)
    if os.time() > start_time and os.time() < end_time then
        ...
        if data.ButtonGet_Game then
            if MainModel.UserInfo.ui_config_id == 2 or true then return end
...
function M.On_EnterScene()
    ...
    if MainModel.UserInfo and MainModel.UserInfo.xsyd_status == 0 then
        return    
    end
