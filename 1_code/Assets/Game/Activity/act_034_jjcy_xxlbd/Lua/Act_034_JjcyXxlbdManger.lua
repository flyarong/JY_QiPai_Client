...
Act_034_JjcyXxlbdManger = {}
local M = Act_034_JjcyXxlbdManger
M.key = "act_034_jjcy_xxlbd"
--GameButtonManager.ExtLoadLua(M.key, "Act_034_JjcyXxlbdEnter")
GameButtonManager.ExtLoadLua(M.key, "Act_034_JjcyXxlbdPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "xiaoxiaole_034_rank"

-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
    --local e_time=1603728000
    --local s_time=1603150200
    local e_time
    local s_time
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_034_JjcyXxlbdPanel.Create(parm.parent, parm.backcall)
        end
...
function M.InitRankData(_data)
    ...
    for i = 1, #_data.rank_data do
        ...
        lis_prop["rank_mult"] = _data.rank_data[i].score / 100
...
function M.InitUserData(_data)
    ...
    user_data["rank_mult"] = _data.score / 100
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=15>JJCY消消乐榜单rank_base_info-----</size></color>")
...
function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=15>JJCY消消乐榜单rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
        --Event.Brocast("act_015_xxlbd_base_info_get",{rank_type = data.rank_type})
    end
...
function M.on_year_btn_created(data)
    if  data and data.enterSelf then
        ...
        notice_prefab=newObject("Act_034_JjcyXxlbdPrefab",btn_gameObject.transform)
