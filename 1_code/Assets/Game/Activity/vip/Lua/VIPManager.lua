...
function M.CheckIsShow()
    return GameGlobalOnOff.Vip
end

function M.GotoUI(parm)
    if not GameGlobalOnOff.Vip then
        return
    end
    ...
    elseif parm.goto_scene_parm == "vip_task_match" then
        local is_have = MatchModel and MatchModel.IsTodayHaveMatchByType("mxb")
...
function M.InitCfg(cfg)
    ...
    m_cfg.yjtz_four = {}
    for i,v in ipairs(cfg.yjtz_four) do
        m_cfg.yjtz_four[v.index] = v
    end
    m_cfg.yjtz_loop = {}
    for i,v in ipairs(cfg.yjtz_loop) do
        m_cfg.yjtz_loop[v.index] = v
    end
    return m_cfg
end
...
function M.PayPanelCreate(tf)
    if GameGlobalOnOff.VIPGift then
        VIPPayPrefab.Create(tf.panelSelf.top_node)
    end
end
...
function M.on_task_change_msg(data)
    if data.id == 21883 and not M.checkBoxYXTZ then
        M.checkBoxYXTZ = true
    elseif data.id == 21244 then
        dump(data, "<color=white>on_task_change_msg______VIP赢金挑战勇者宝箱______</color>")
        M.boxChangeYXTZ = true
        M.HandleCheckAndShoWBoxYJTZ()
    end
    if not M.check_is_vip_task(data.id) then return end
...
function M.get_vip_data()
    if not m_data or not m_data.vip_data then
        return
    end
...
function M.set_vip_text(txt,vip_level)
    ...
    if GameGlobalOnOff.Vip then
        if txt.font.name == "vip_font_ty_hz" or txt.font.name == "vip_font_ty_hz_1" or string.match(txt.font.name,"vip_font_ty_hz") or string.match(txt.font.name,"vip_font_ty_hz_1") then
            if vip_level then
...
function M.CheakRed_vipyjtz()
    ...
    or (M.get_vip_task(21315) and M.get_vip_task(21315).award_status == 1)
    or (M.get_vip_task(21651) and M.get_vip_task(21651).award_status == 1) 
    or (M.get_vip_task(21883) and M.get_vip_task(21883).award_status == 1) then 
        return true
    else
        return false
    end 
...
function IsFinishTask(task_id)
    td = GameTaskModel.GetTaskDataByID(task_id)
    if td and (td.award_status == 2 or td.award_status == 3) then
        return true
    end
end

function M.GetCurStateYKTZ()
    local cur_state = 0
    for i = 1, #yjtz_task do
        if not IsFinishTask(yjtz_task[i]) then
            cur_state = i
            break
        end
    end
...
function M.GetCurTaskYJTZ()
    local cur_state = M.GetCurStateYKTZ()
    return yjtz_task[cur_state]
end

function M.GetFinishTaskYJTZ()
    ...
    return yjtz_task[finish_state]
end

local box_task_id = 21244
local box_text = {
    [1] = "感谢你对游戏的支持，\n在踏上新征途之前，请收下我们为您准备的宝箱",
    [2] = "真了不起！恭喜您已完成第二阶段的赢金挑战，\n点击领取我们为你准备的勇者宝箱",
    [3] = "真了不起！恭喜您已完成第三阶段的赢金挑战，\n点击领取我们为你准备的勇者宝箱",
    [4] = "真了不起！恭喜您已完成第四阶段的赢金挑战，\n点击领取我们为你准备的勇者宝箱",
    [5] = "真了不起！恭喜您完成本期赢金挑战，\n点击领取我们为你准备的勇者宝箱",
}
M.boxChangeYXTZ = false

function M.CheckAndShoWBoxYJTZ()
    M.checkBoxYXTZ = true
    M.HandleCheckAndShoWBoxYJTZ()
end
--检查并显示勇者宝箱
function M.HandleCheckAndShoWBoxYJTZ()
    if not M.boxChangeYXTZ or not M.checkBoxYXTZ then
        -- if not M.boxChangeYXTZ then
        --     dump("<color=white>勇者宝箱数据未更新</color>")
        -- end
        -- if not M.checkBoxYXTZ then
        --     dump("<color=white>勇者宝箱还未开始检测</color>")
        -- end
        return 
    end
    local boxTaskData = GameTaskModel.GetTaskDataByID(21244)
    dump(boxTaskData, "<color=white>勇者宝箱数据Data</color>")
    if not boxTaskData then
        return
    end
    local b = basefunc.decode_task_award_status(boxTaskData.award_get_status)
    b = basefunc.decode_all_task_award_status(b, boxTaskData, boxTaskData.now_lv)
    dump(b, "<color=white>勇者宝箱数据BBB</color>")
    local finishIndex = M.GetCurStateYKTZ() - 1
    dump(finishIndex, "<color=white>显示完成第几个阶段finishIndex</color>")

    for i = 1, #b do
        local isCanShow = false
        if b[i] == 1 then
            if i >= 5 or (i < 5 and i == finishIndex) then
                isCanShow = true
            end
        end
        if isCanShow then
            ...
            local str = box_text[textIndex]
            VIPLJYJ88GetPanel.Create({dec = str})
            M.boxChangeYXTZ = false
            M.checkBoxYXTZ = false
            break
        end
    end
end
...
--[[
    GetTexture("vip_tq_icon_hz1")
    GetTexture("vip_tq_icon_hz2")
    GetTexture("vip_tq_icon_hz3")
    GetTexture("vip_tq_icon_hz4")
    GetTexture("vip_tq_icon_hz5")
    GetTexture("vip_tq_icon_hz6")
    GetTexture("vip_tq_icon_hz7")
    GetTexture("vip_tq_icon_hz8")
    GetTexture("vip_tq_icon_hz9")
    GetTexture("vip_tq_icon_hz10")
    GetTexture("vip_tq_icon_hz11")
    GetTexture("vip_tq_icon_hz12")
    GetTexture("vip_ztlb_icon")
    GetTexture("vip_zx_icon_1")
    GetTexture("vip_zx_icon_2")
    GetTexture("vip_zx_icon_3")
    GetTexture("vip_zx_icon_4")
    GetTexture("vip_zx_icon_5")
    GetTexture("vip_zx_icon_6")
]]