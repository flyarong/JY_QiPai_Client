...
Act_017_HYZHManager = {}
local M = Act_017_HYZHManager
M.key = "act_017_hyzh"
M.config = GameButtonManager.ExtLoadLua(M.key, "act_017_hyzh_config") 
GameButtonManager.ExtLoadLua(M.key, "Act_017_HYZHPanel")
local lister
M.task_id = 21593
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_017_HYZHPanel.Create(parm.parent)
    end 
...
function M.OnLoginResponse(result)
    if result == 0 then
        ...
        dump(M.task_id,"<color=white>================================</color>")
