...
Act_006XYFLManager = basefunc.class()
local M = Act_006XYFLManager
M.key = "act_006_xyfl"
local config = GameButtonManager.ExtLoadLua(M.key, "act_006_xyfl_config")
local lister
local task_ids
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_p_wealth_eliminate", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
