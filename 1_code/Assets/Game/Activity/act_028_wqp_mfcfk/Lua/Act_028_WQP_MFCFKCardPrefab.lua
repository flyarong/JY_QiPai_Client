-- 创建时间:2020-06-18
-- Panel:Act_028_WQP_MFCFKCardPrefab
--[[
*      ┌─┐       ┌─┐
*   ┌──┘ ┴───────┘ ┴──┐
*   │                 │
...
Act_028_WQP_MFCFKCardPrefab = basefunc.class()
local C = Act_028_WQP_MFCFKCardPrefab
C.name = "Act_028_WQP_MFCFKCardPrefab"
...
function C:MakeLister()
	self.lister = {}
	self.lister["act_028_call_other_card"] = basefunc.handler(self,self.act_028_call_other_card)
	self.lister["Act_028_WQP_MFCFK_info_get"] = basefunc.handler(self,self.Act_028_WQP_MFCFK_info_get)
	self.lister["Act_028_WQP_MFCFK_can_click"] = basefunc.handler(self,self.Act_028_WQP_MFCFK_can_click)
end
...
function C:FzAnim()
	...
	self.seq:AppendCallback(function ()
		if self.is_click then
			...
			print("已经翻转")
			Event.Brocast("Act_028_WQP_MFCFK_can_quit")
		end
	end)
		...
		function ()
			if self.award_data then
				...
				local timer = Timer.New(function ()
					...
					elseif t == 1 then
						Event.Brocast("act_028_call_other_card")
					end
				end,1,4,false)
				timer:Start()
			end
...
function C:SetAwardData()
	if not self.is_click then
		local data =  Act_028_WQP_MFCFKGetAwardPanel.GetFakeAward()
...
function C:Act_028_WQP_MFCFK_info_get(data)
...
function C:OnBtnClick()
	if self.can_click then
		Network.SendRequest("get_task_award_new",{id = Act_028_WQP_MFCFKPanel.Now_Task_ID,award_progress_lv = self.index})
		self.is_click = true
		if M.IsFirstGetAward() then
			M.SetFirstGetAward()
		end
		Event.Brocast("Act_028_WQP_MFCFK_can_click",{can_click = false})
	end
end
...
function C:act_028_call_other_card()
	if not self.is_click then
		self:FzAnim()
	end
end

function C:Act_028_WQP_MFCFK_can_click(data)
