-- 创建时间:2020-06-18
-- Panel:CPL_LJYJCFKGetAwardPanel
--[[
 *      ┌─┐       ┌─┐
 *   ┌──┘ ┴───────┘ ┴──┐
 *   │                 │
...
CPL_LJYJCFKGetAwardPanel = basefunc.class()
local C = CPL_LJYJCFKGetAwardPanel
C.name = "CPL_LJYJCFKGetAwardPanel"
local had_use = {}
...
function C:MakeLister()
	self.lister = {}
	self.lister["CPL_LJYJCFK_can_quit"] = basefunc.handler(self,self.CPL_LJYJCFK_can_quit)
...
function C:MyExit()
	...
	if not M.CanGetAwardIndex() then
		Event.Brocast("close_CPL_LJYJCFK")
	end
	self:RemoveListener()
...
function C:InitUI()
	self.prefabs = {}
	for i = 1,5 do
		self.prefabs[#self.prefabs + 1] = CPL_LJYJCFKCardPrefab.Create(self["node"..i],self.index)
	...
	self:GuideAnim(self.prefabs,function ()
		Event.Brocast("CPL_LJYJCFK_can_click",{can_click = true})
...
function C:OnAssetChange(data)
    ...
    if data.change_type and data.change_type == "task_p_freestyle_ddz" then
		...
		Event.Brocast("CPL_LJYJCFK_info_get",{award_data = data})
    end
end
...
function C:CPL_LJYJCFK_can_quit()
