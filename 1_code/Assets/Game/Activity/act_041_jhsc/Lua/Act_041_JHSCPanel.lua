...
Act_041_JHSCPanel = basefunc.class()
local C = Act_041_JHSCPanel
C.name = "Act_041_JHSCPanel"
local M = Act_041_JHSCManager
C.instance = nil

local help_info1 = {
"1.活动时间：12月8日7:30~12月14日23:59:59",
"2.活动期间街机捕鱼小游戏中可获得礼物道具",
"3.活动结束后所有未使用的礼物道具将被清除，请及时兑换",
...
function C:MakeLister()
	...
	self.lister["model_jhsc_data_change_msg"] = basefunc.handler(self,self.MyRefresh)
	self.lister["jhsc_toggle_set_true_msg"] = basefunc.handler(self,self.on_jhsc_toggle_set_true_msg)
	self.lister["jhsc_toggle_set_false_msg"] = basefunc.handler(self,self.on_jhsc_toggle_set_false_msg)
end
...
function C:MyExit()
	if self.jhsc_timer then
		self.jhsc_timer:Stop()
		self.jhsc_timer = nil
	end
...
function C:CreateItemPrefab()
	...
	for i=1,#self.cur_data do
		local pre = Act_041_JHSCItemBase.Create(self.Content.transform,self.cur_data[i])
		if pre then
...
function C:on_jhsc_toggle_set_true_msg()
	self.xqdh_timer = Timer.New(function ()
		self.Toggle_tge.gameObject:SetActive(true)
	end,0.1,1,true)
	self.xqdh_timer:Start()
end

function C:on_jhsc_toggle_set_false_msg()
	if self.Toggle_tge.isOn then
		...
		et = et + 1
		PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."jhsc",et)
	end
