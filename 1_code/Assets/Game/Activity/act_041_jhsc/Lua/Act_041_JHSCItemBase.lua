...
Act_041_JHSCItemBase = basefunc.class()
local C = Act_041_JHSCItemBase
C.name = "Act_041_JHSCItemBase"
...
function C:MakeLister()
	...
	self.lister["jhsc_sw_kfPanel_msg"] = basefunc.handler(self,self.on_jhsc_sw_kfPanel_msg)
end
...
function C:on_enough_BuyClick()
	if os.time() >= PlayerPrefs.GetInt(MainModel.UserInfo.user_id.."jhsc") then
		Event.Brocast("jhsc_toggle_set_true_msg")
		HintPanel.Create(2,"是否兑换"..self.data.award_name.."？",function ()
			Event.Brocast("jhsc_toggle_set_false_msg")
...
function C:on_jhsc_sw_kfPanel_msg(id)
	if id == self.data.ID then
		if self.data.type == 1 then
			local real = {}
			real.text = M.config.Info[4].award_name
			real.image = M.config.Info[4].award_image
