...
M.key = "act_041_jhsc"
Act_041_JHSCManager.config = GameButtonManager.ExtLoadLua(M.key,"act_041_jhsc_config")
GameButtonManager.ExtLoadLua(M.key,"Act_041_JHSCItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_041_JHSCPanel")
local this
local lister
local gift_ids
local task_map_ids
...
M.type = 20
M.now_level = 0
M.item_key = "prop_zhshe_giftbox" --礼物
--money "106306","prop_zhshe_giftbox",100000

local limit_icons = 
{
    "xqdh_icon_xl1_activity_act_041_jhsc",
    "xqdh_icon_xl22",
    "xqdh_icon_xl3_activity_act_041_jhsc"
}

-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1607961599
    local s_time = 1607383800
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_041_JHSCPanel.Create(parm.parent,parm.backcall)
        end 
    else
...
function M.Refresh()
    ...
    Event.Brocast("model_jhsc_data_change_msg")--刷新panel
end
...
function M.QueryGiftData()
    ...
    if gift_data and CheckGiftDataFinish() and (os.time() - this.m_data.time < 5) then
        Event.Brocast("model_jhsc_data_change_msg")
    else
        M.query_data()
    end
end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data then
        if data.result == 0 then
            M.query_data()
            Event.Brocast("jhsc_sw_kfPanel_msg",data.id)
        else
            HintPanel.ErrorMsg(data.result)
        end
    end
