...
ACT_010_51FLSYYManager = {}
local M = ACT_010_51FLSYYManager
M.key = "act_010_51flsyy"
GameButtonManager.ExtLoadLua(M.key, "ACT_010_51FLSYYPanel") 
GameButtonManager.ExtLoadLua(M.key, "ACT_010_51FLSYYPanel_Out") 
local lister
local m_data
local activity_id_year = 66
local activity_id_game = 126
local activity_ID_game = 137
local is_yy = false
local end_time = 1588424400
local start_time = 1588028400
local match_start_time = 1588424400
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return ACT_010_51FLSYYPanel.Create(parm.parent)
...
function M.on_act_match_order_msg_change(data)
    ...
    --只在当日做变化
    if os.time() > 1588348800 and os.time() < match_start_time then 
    --大厅跳转
    ...
    if os.time() > start_time and os.time() < end_time then
        if not is_yy and MainModel.GetHBValue() >= 1 then 
            ...
            if oldtime ~= newtime then
                ACT_010_51FLSYYPanel_Out.Create()
