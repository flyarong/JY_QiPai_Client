...
SN_SHFLManager = basefunc.class()
local M = SN_SHFLManager
M.key = "act_sn_shfl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_mouse_shxxl_config")
local lister
local task_ids
local function MakeLister()
    ...
    lister["model_query_one_task_data_response"] = M.Refresh_Status
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21108", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
