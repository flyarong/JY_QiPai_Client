...
M.key = "act_024_bzdh"
local prefab
GameButtonManager.ExtLoadLua(M.key,"Act_024_BZDHPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_024_BZDHPrefab")
--"♠ : 1","♥ : 2","♣ : 3","♦ : 4"
M.parm = {
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_024_BZDHPanel.Create(parm.parent)
        end
...
function M.EnterScene()
	...
	if MainModel.myLocation == "game_Fishing" then
        prefab = Act_024_BZDHPrefab.Create()
