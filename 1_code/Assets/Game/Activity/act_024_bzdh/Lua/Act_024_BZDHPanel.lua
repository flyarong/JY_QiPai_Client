...
Act_024_BZDHPanel = basefunc.class()
local C = Act_024_BZDHPanel
C.name = "Act_024_BZDHPanel"
local M = Act_024_BZDHManager
--"♠ : 1","♥ : 2","♣ : 3","♦ : 4",46-54
local ex_change = {
	...
	{ex_change_id = 104,need = {{parm = 3,num =2},{parm = 4,num =2}},award ={{num = 90,_type = "jifen"},{num = 3000,_type = "jing_bi"},{num = 30,_type = "jifen"}}},
	{ex_change_id = 105,need = {{parm = 1,num =2},{parm = 2,num =2}},award ={{num = 200,_type = "jifen"},{num = 6000,_type = "jing_bi"},{num = 60,_type = "jifen"}}},
}
...
function C:InitUI()
	self.duihuan_btn.onClick:AddListener(function ()
		...
		HintPanel.Create(2,"一键兑换功能会根据花色组合从多到少自动匹配兑换，是否确认兑换",function ()
			Network.SendRequest("box_all_exchange",{name = "all_treasury_exchange"})
		end)
...
function C:InitMainUI()
	...
	for i = 1,#ex_change do
		...
		for m = 1,#ex_change[i].award do
			...
			if m == 1 then
				PointerEventListener.Get(temp_ui["award"..m.."_img"].gameObject).onDown = function ()
					GameTipsPrefab.ShowDesc("用于参加船长争霸活动！", UnityEngine.Input.mousePosition)
				end
				PointerEventListener.Get(temp_ui["award"..m.."_img"].gameObject).onUp = function ()
					GameTipsPrefab.Hide()
				end
