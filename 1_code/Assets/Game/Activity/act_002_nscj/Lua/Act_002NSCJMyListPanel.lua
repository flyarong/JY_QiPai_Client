...
Act_002NSCJMyListPanel = basefunc.class()
local C = Act_002NSCJMyListPanel
C.name = "Act_002NSCJMyListPanel"
local Page_Index  =  1
function C.Create()
	return C.New()
end
...
function C:ctor()
	...
	self:AddMsgListener()
	self:InitUI()
	Network.SendRequest("common_lottery_query_my_award_logs",{lottery_type = "box_exchange_13",is_shiwu = 1})
end
...
function C:onGetInfo(_,data)
	...
	if data and data.result == 0 then
		local task_data = GameTaskModel.GetTaskDataByID(21170)
		if task_data then 
			if task_data.award_status == 2 then 
				data.award_logs[#data.award_logs + 1] = {award_name = "海王香水",count = 1}
			end 
		end  
