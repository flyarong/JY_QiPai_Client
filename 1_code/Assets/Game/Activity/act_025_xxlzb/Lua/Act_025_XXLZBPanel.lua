...
Act_025_XXLZBPanel = basefunc.class()
local C = Act_025_XXLZBPanel
C.name = "Act_025_XXLZBPanel"
local M = Act_025_XXLZBManager
local HGList={
	[1] ="localpop_icon_1",
	[2] ="localpop_icon_2",
	[3] ="localpop_icon_3",
...
local base_txt = {
	[1] = {"任意消消乐游戏<color=yellow><size=35>3万</size></color>及以上档次且获得倍数大于等于5倍，可参与排行榜","活动时间：8月18日7:30-8月24日23:59:59"},
}
local type_color = {
...
function C:onMyInfoGet()
	...
	if data and data.result ==  0 and IsEquals(self.gameObject)  then
		self["my"..curr_index.."_num_txt"].text = (data.score/100).."倍"
...
function C:onInfoGet(data)
	...
	if data and data.result ==0 and IsEquals(self.gameObject) then 
		for i = 1, #data.rank_data do
			...
			self["I"..curr_index.."_num_txt"].text = (data.rank_data[i].score/100).."倍"
			if json_data then
				local txt = ""
