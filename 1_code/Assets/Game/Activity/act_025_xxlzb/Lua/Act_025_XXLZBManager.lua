-- 创建时间:2020-05-25
-- Act_025_XXLZBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_025_XXLZBManager = {}
local M = Act_025_XXLZBManager
M.key = "act_025_xxlzb"
GameButtonManager.ExtLoadLua(M.key, "Act_025_XXLZBPanel")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_025_XXLZBPanel.Create(parm.parent)
        end
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_025_XXLZBPrefab",btn_gameObject.transform)
