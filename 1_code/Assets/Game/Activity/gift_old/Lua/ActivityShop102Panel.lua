...
ActivityShop102Panel = basefunc.class()
local C = ActivityShop102Panel
C.name = "ActivityShop102Panel"
...
function C:UpdateTime()
	...
	if IsEquals(self.TimeText) and self.gift_config then
		self.TimeText.text = "礼包下架时间：" .. os.date("%m月%d日%H:%M", self.gift_config.end_time)
	end
