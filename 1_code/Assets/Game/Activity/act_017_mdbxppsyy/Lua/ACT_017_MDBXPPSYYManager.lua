...
ACT_017_MDBXPPSYYManager = {}
local M = ACT_017_MDBXPPSYYManager
M.key = "act_017_mdbxppsyy"
GameButtonManager.ExtLoadLua(M.key, "ACT_017_MDBXPPSYYPanel") 
GameButtonManager.ExtLoadLua(M.key, "ACT_017_MDBXPPSYYPanel_Out") 
local lister
local m_data
local activity_id_year = 92
local activity_id_game = 142
local activity_ID_game = 153
local is_yy = false
local end_time = 1593446399
local start_time = 1592868600
local match_start_time = 1593522000
local mathc_day_time = 1593446400--比赛当天零点,比如比赛当天是5月1日21:00点.那么这个值就是5月1日00:00点
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return ACT_017_MDBXPPSYYPanel.Create(parm.parent)
    ...
    if os.time() > start_time and os.time() < end_time then
        if not is_yy and MainModel.GetHBValue() >= 1 then 
            ...
            if oldtime ~= newtime then
                ACT_017_MDBXPPSYYPanel_Out.Create()
