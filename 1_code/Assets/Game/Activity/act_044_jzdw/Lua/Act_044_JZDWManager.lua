...
Act_044_JZDWManager = {}
local M = Act_044_JZDWManager
M.key = "act_044_jzdw"
GameButtonManager.ExtLoadLua(M.key, "Act_044_JZDWPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "hqyd_044_jzdw_rank"

-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
    local s_time = 1609198200 --20.12.29
    local e_time = 1609775999 --21.01.02
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_044_JZDWPanel.Create(parm.parent, parm.backcall)
        end
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=14>饺子大王rank_base_info-----</size></color>")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        M.InitUserData(data)
        Event.Brocast("act_044_jzdw_rank_base_info_get",{rank_type = data.rank_type})
    end
end

function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=14>饺子大王rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
        Event.Brocast("act_044_jzdw_rank_info_get",{rank_type = data.rank_type})
    end
end
...
function M.on_year_btn_created(data)
    if  data and data.enterSelf then
        ...
        notice_prefab=newObject("Act_044_JZDWPrefab",btn_gameObject.transform)
