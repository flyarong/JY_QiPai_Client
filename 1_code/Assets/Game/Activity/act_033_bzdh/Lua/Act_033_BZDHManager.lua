...
M.key = "act_033_bzdh"
local prefab
GameButtonManager.ExtLoadLua(M.key,"Act_033_BZDHPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_033_BZDHPrefab")
--"♠ : 1","♥ : 2","♣ : 3","♦ : 4"
M.parm = {
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_033_BZDHPanel.Create(parm.parent)
        end
...
function M.EnterScene()
	...
	if MainModel.myLocation == "game_Fishing" and is_nor then
        prefab = Act_033_BZDHPrefab.Create()
