...
Act_033_SWITCHPanel = basefunc.class()
local C = Act_033_SWITCHPanel
C.name = "Act_033_SWITCHPanel"
...
function C:ctor(parent,backcall)
	...
	if next(self.config) then
		if Act_033_BZZBManager.GetBestRank() <= 20 then
...
function C:on_had_switch_btn(parm)
	if parm then
		...
		if parm.gotoui == "act_033_bzdh" then
...
function C:WorkBtn(temp_ui,manager_str)
	if manager_str == "act_033_bzdh" then
		dump(temp_ui)
		temp_ui.main_img.sprite = GetTexture("bzdh_imgf_bzdh1_new")
		temp_ui.mask_img.sprite  = GetTexture("bzdh_imgf_bzdh2_new")			
	else
		...
		temp_ui.main_img.sprite  = GetTexture("bzdh_imgf_bzxzb1_new")
		temp_ui.mask_img.sprite  = GetTexture("bzdh_imgf_bzxzb2_new")
	end
end
