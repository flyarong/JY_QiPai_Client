...
Act_042_YGSJBManager = {}
local M = Act_042_YGSJBManager
M.key = "act_042_ygsjb"
GameButtonManager.ExtLoadLua(M.key, "Act_042_YGSJBPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "dzswn_042_ygsjb_rank"
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_042_YGSJBPanel.Create(parm.parent, parm.backcall)
        end
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=14>阳光收集榜rank_base_info-----</size></color>")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        M.InitUserData(data)
        Event.Brocast("act_042_ygsjb_rank_base_info_get",{rank_type = data.rank_type})
    end
end

function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=14>阳光收集榜rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
        Event.Brocast("act_042_ygsjb_rank_info_get",{rank_type = data.rank_type})
    end
end
...
function M.on_year_btn_created(data)
    if  data and data.enterSelf then
        ...
        notice_prefab=newObject("Act_042_YGSJBPrefab",btn_gameObject.transform)
