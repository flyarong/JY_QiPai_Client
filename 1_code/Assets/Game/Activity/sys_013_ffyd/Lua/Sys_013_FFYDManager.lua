...
function M.on_hallpanel_created(data)
    ...
    if data and data.panelSelf and IsEquals(data.panelSelf.MINIGAMEBox) then 
...
function M.On_EnterScene()
    --从匹配转到大厅
    if MainModel.lastmyLocation == "game_Free" and MainModel.myLocation == "game_Hall" and (MainModel.UserInfo.xsyd_status == 1 or not GameGlobalOnOff.IsOpenGuide)then
