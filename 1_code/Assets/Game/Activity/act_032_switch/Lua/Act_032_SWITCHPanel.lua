...
Act_032_SWITCHPanel = basefunc.class()
local C = Act_032_SWITCHPanel
C.name = "Act_032_SWITCHPanel"
local M = Act_032_SWITCHManager

local DESCRIBE_TEXT = {
	"1.活动时间：10月6日7:30~10月12日23:59:59",
	"2.3万及以上档次消除时可获得消消乐元素。",
	"3.所有元素只记录普通消除时的元素数量",
	'4.当日购买"消除礼包"，兑换时可获得礼包积分，仅当日有效',
}
...
function C:on_had_switch_btn(parm)
	if parm then
		...
		if parm.gotoui == "act_032_xxldh" then
...
function C:WorkBtn(temp_ui,manager_str)
	if manager_str == "act_032_xxldh" then
		dump(temp_ui)
		temp_ui.main_img.sprite = GetTexture("xchl_imgf_xchl1")
		temp_ui.mask_img.sprite  = GetTexture("xchl_imgf_xchl2")			
	else
		...
		temp_ui.main_img.sprite  = GetTexture("xchl_imgf_xxlzb1")
		temp_ui.mask_img.sprite  = GetTexture("xchl_imgf_xxlzb2")
	end
end
