-- 创建时间:2020-06-09
-- Act_025_ExSJSHManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_025_ExSJSHManager = {}
local M = Act_025_ExSJSHManager
M.key = "act_025_exsjshl"
...
function M.FlyAnim(obj)
    ...
    if true then
        ...
        local seq = DoTweenSequence.Create(true)
        local path = {}
        path[0] = a
...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and data.change_type == "task_award_prop_imperial" then
...
function M.PrefabCreator(value)
    ...
    local obj = newObject("Act_025_SJSHLPrefab",GameObject.Find("Canvas/LayerLv50").transform)
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_award_prop_imperial" == type then
            return true
        end
    end
