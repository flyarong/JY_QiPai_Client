...
Act_028_DJJTLBPanel = basefunc.class()
local C = Act_028_DJJTLBPanel
C.name = "Act_028_DJJTLBPanel"
...
function C:InitUI()
	...
	PointerEventListener.Get(self["wan"].gameObject).onDown = function ()
		GameTipsPrefab.ShowDesc("万能图可替代任意图，兑换时自动使用", UnityEngine.Input.mousePosition)
	end
...
function C:MyRefresh()
	...
	if s == 1 then
		...
		if os.time() >= 1600012800 then
			self.mask_txt.text = "已购买"
		else
			self.mask_txt.text = "明日购买"
		end
	end
end
