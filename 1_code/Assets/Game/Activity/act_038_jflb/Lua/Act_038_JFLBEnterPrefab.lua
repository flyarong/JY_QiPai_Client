...
Act_038_JFLBEnterPrefab = basefunc.class()
local C = Act_038_JFLBEnterPrefab
C.name = "Act_038_JFLBEnterPrefab"
...
function C:ctor(parent)
	...
	local obj = newObject("Act_038_JFLBEnterPrefab", parent)
...
function C:OnEnterClick()
	local b = GameComGiftT4Panel.Create(nil, nil, M.config[M.level],nil,"Act_038_JFLBPanel")
	for i = 1,3 do
		local ui = {}
		LuaHelper.GeneratingVar(b["gift_rect"..i], ui)
		PointerEventListener.Get(ui.icon3_img.gameObject).onDown = function ()
			LTTipsPrefab.Show2(ui.icon3_img.gameObject.transform,"积分礼包","购买一个礼包，积分1.2倍领取；\n购买两个礼包，积分1.5倍领取；\n购买三个礼包，积分2倍领取。")
		end
		PointerEventListener.Get(ui.icon3_img.gameObject).onUp = function ()
			LTTipsPrefab.Hide()
		end
	end
	b.help_btn.onClick:AddListener(
		function()
			ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
