-- 创建时间:2020-06-02
-- Act_016_CJLBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_016_CJLBManager = {}
local M = Act_016_CJLBManager
M.key = "act_016_cjlb"
Act_016_CJLBManager.config = GameButtonManager.ExtLoadLua(M.key,"act_016_cjlb_config")
GameButtonManager.ExtLoadLua(M.key,"Act_016_CJLBEnterPrefab")
GameButtonManager.ExtLoadLua(M.key,"Act_016_CJLBPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_016_CJLBItemBase")
local this
local lister
local permisstions = {
    "actp_buy_gift_bag_class_lottery_1",
    "actp_buy_gift_bag_class_lottery_2",
    "actp_buy_gift_bag_class_lottery_3",
}
...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1592841599
    local s_time = 1592263800
...
function M.on_ActivityYearPanel_Had_Finish(parm)
    if M.CheckIsShow() and parm then
        Act_016_CJLBEnterPrefab.Create(parm.panelSelf.transform)
    end
end
