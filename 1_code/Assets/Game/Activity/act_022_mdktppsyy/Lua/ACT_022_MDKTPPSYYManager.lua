...
ACT_022_MDKTPPSYYManager = {}
local M = ACT_022_MDKTPPSYYManager
M.key = "act_022_mdktppsyy"
GameButtonManager.ExtLoadLua(M.key, "ACT_022_MDKTPPSYYPanel") 
GameButtonManager.ExtLoadLua(M.key, "ACT_022_MDKTPPSYYPanel_Out") 
local lister
local m_data
local activity_id_year = 107
local activity_id_game = 152
local activity_ID_game = 163
local is_yy = false
local end_time = 1596286800
local start_time = 1595892600
local match_start_time = 1596286800
local mathc_day_time = 1596211200--比赛当天零点,比如比赛当天是5月1日21:00点.那么这个值就是5月1日00:00点
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return ACT_022_MDKTPPSYYPanel.Create(parm.parent)
...
function M.on_act_match_order_msg_change(data)
    ...
    --只在当日做变化
    if os.time() > mathc_day_time and os.time() < match_start_time then 
    --大厅跳转
    ...
    if os.time() > start_time and os.time() < end_time then
        if not is_yy and MainModel.GetHBValue() >= 1 then 
            ...
            if oldtime ~= newtime then
                ACT_022_MDKTPPSYYPanel_Out.Create()
