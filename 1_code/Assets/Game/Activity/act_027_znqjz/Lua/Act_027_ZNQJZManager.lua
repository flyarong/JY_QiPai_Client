-- 创建时间:2020-08-14
-- Act_027_ZNQJZManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_027_ZNQJZManager = {}
...
GameButtonManager.ExtLoadLua(M.key,"Act_027_ZNQJZPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_027_ZNQJZLBPanel")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_027_ZNQJZPanel.Create(parm.parent)
    end
...
function M.IsCanGetAward()
    ...
    if need_type_num == 0 or GameItemModel.GetItemCount(M.item_keys[5]) >= need_type_num then
        return true
    else
        return false 
    end
...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and data.change_type == "task_duiju_collect_world" then
...
function M.PrefabCreator(value,_type)
    ...
    local obj = newObject("Act_027_ZNQJZPrefab",GameObject.Find("Canvas/LayerLv50").transform)
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
    obj.transform.position = Vector3.New(0,550,0)
    LuaHelper.GeneratingVar(obj.transform, temp_ui)
    local _type2img = {
        prop_2year_zhou = "djjzhhl_icon_znqd1",
        prop_2year_nian = "djjzhhl_icon_znqd2",
        prop_2year_qing = "djjzhhl_icon_znqd3",
        prop_2year_dian = "djjzhhl_icon_znqd4",
        prop_2year_wan  = "djjzhhl_icon_znqd5",
    }
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_duiju_collect_world" == type then
            return true
        end
    end
