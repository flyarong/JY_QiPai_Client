...
M.key = "act_033_bzzb"
GameButtonManager.ExtLoadLua(M.key, "Act_033_BZZBPanel")
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_033_BZZBPanel.Create(parm.parent)
        end
    end 
...
function M.query_rank_base_info_response(_,data)
    if data and data.result == 0 then
        ...
        Event.Brocast("act_033_base_info_get",{rank_type = data.rank_type})
    end
end
