...
M.key = "act_038_jfzb"
GameButtonManager.ExtLoadLua(M.key, "Act_038_JFZBPanel")
local this
local lister
...
local notice_timer
local best_rank = 1
M.base_type = "znqd_3rd_qdbd_rank" 
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1632758399 --2021年11月23日23时59分59秒
    local s_time = 1632180600 --2020年11月17日7时30
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_038_JFZBPanel.Create(parm.parent)
        end
    end 
...
function M.OnLoginResponse(result)
	if result == 0 then
        -- 数据初始化      
        Network.SendRequest("query_rank_base_info",{rank_type = M.base_type})
	end
end
...
function M.query_rank_base_info_response(_,data)
    if data and data.result == 0 then
        ...
        Event.Brocast("act_038_base_info_get",{rank_type = data.rank_type})
    end
end
...
function M.GetExtCfg()
    local isOpen, cfg = GameButtonManager.RunFun({gotoui = "act_ty_rank"}, "GetExtCfg")
    if isOpen then
        return cfg
    end
end
