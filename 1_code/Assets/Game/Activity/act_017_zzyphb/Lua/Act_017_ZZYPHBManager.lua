...
Act_017_ZZYPHBManager = {}
local M = Act_017_ZZYPHBManager
M.key = "act_017_zzyphb"
GameButtonManager.ExtLoadLua(M.key, "Act_017_ZZYPHBPanel")
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_017_ZZYPHBPanel.Create(parm.parent)
        end
    end 
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_017_ZZYPHBPrefab",btn_gameObject.transform)
