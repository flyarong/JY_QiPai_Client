...
Act_024_SWITCHPanel = basefunc.class()
local C = Act_024_SWITCHPanel
C.name = "Act_024_SWITCHPanel"
local M = Act_024_SWITCHManager
...
function C:ctor(parent,backcall)
	...
	if next(self.config) then
		if Act_024_CZZBManager.GetBestRank() <= 20 then
...
function C:on_had_switch_btn(parm)
	if parm then
		...
		if parm.gotoui == "act_024_bzdh" then
...
function C:RefreshDefaultPanel(manager_str)
	for k,v in pairs(self.default_panels) do
		if k == manager_str then
...
function C:WorkBtn(temp_ui,manager_str)
	if manager_str == "act_024_bzdh" then
		dump(temp_ui)
		temp_ui.main_img.sprite = GetTexture("bzdh_imgf_bzdh1_activity_act_024_switch")
		temp_ui.mask_img.sprite  = GetTexture("bzdh_imgf_bzdh2_activity_act_024_switch")			
	else
		...
		temp_ui.main_img.sprite  = GetTexture("bzdh_imgf_czzb1")
		temp_ui.mask_img.sprite  = GetTexture("bzdh_imgf_czzb2")
	end
end
