-- 创建时间:2020-05-25
-- Act_028_JFDRBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_028_JFDRBManager = {}
local M = Act_028_JFDRBManager
M.key = "act_028_jfdrb"
GameButtonManager.ExtLoadLua(M.key, "Act_028_JFDRBPanel")
...
M.base_types = {
    "anniversary_celebration_score_rank",
}
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_028_JFDRBPanel.Create(parm.parent)
        end
    end 
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_028_JFDRBPrefab",btn_gameObject.transform)
