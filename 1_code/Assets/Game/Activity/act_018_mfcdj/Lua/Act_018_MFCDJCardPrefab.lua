...
Act_018_MFCDJCardPrefab = basefunc.class()
local C = Act_018_MFCDJCardPrefab
C.name = "Act_018_MFCDJCardPrefab"
...
function C:MakeLister()
	self.lister = {}
	self.lister["act_018_call_other_card"] = basefunc.handler(self,self.act_018_call_other_card)
	self.lister["act_018_mfcdj_info_get"] = basefunc.handler(self,self.act_018_mfcdj_info_get)
	self.lister["act_018_mfcdj_can_click"] = basefunc.handler(self,self.act_018_mfcdj_can_click)
end
...
function C:FzAnim()
	...
	self.seq:AppendCallback(function ()
		if self.is_click then
			...
			print("已经翻转")
			Event.Brocast("act_018_mfcdj_can_quit")
		end
	end)
		...
		function ()
			if self.award_data then
				...
				local timer = Timer.New(function ()
					...
					elseif t == 1 then
						Event.Brocast("act_018_call_other_card")
					end
				end,1,4,false)
				timer:Start()
			end
...
function C:SetAwardData()
	if not self.is_click then
		local data =  Act_018_MFCDJGetAwardPanel.GetFakeAward()
...
function C:act_018_mfcdj_info_get(data)
...
function C:OnBtnClick()
	if self.can_click then
		...
		self.is_click = true
		Event.Brocast("act_018_mfcdj_can_click",{can_click = false})
	end
end
...
function C:act_018_call_other_card()
	if not self.is_click then
		self:FzAnim()
	end
end

function C:act_018_mfcdj_can_click(data)
