...
Act_042_YGHHLItemBase = basefunc.class()
local C = Act_042_YGHHLItemBase
C.name = "Act_042_YGHHLItemBase"
...
function C:MakeLister()
	...
	self.lister["yghhl_sw_kfPanel_msg"] = basefunc.handler(self,self.on_yghhl_sw_kfPanel_msg)
end
...
function C:on_enough_BuyClick()
	if os.time() >= PlayerPrefs.GetInt(MainModel.UserInfo.user_id.."yghhl") then
		Event.Brocast("yghhl_toggle_set_true_msg")
		HintPanel.Create(2,"是否兑换"..self.data.award_name,function ()
			Event.Brocast("yghhl_toggle_set_false_msg")
...
function C:on_yghhl_sw_kfPanel_msg(id)
