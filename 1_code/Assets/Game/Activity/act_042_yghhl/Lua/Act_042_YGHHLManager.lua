...
Act_042_YGHHLManager = {}
local M = Act_042_YGHHLManager
M.key = "act_042_yghhl"
Act_042_YGHHLManager.config = GameButtonManager.ExtLoadLua(M.key,"act_042_yghhl_config")
GameButtonManager.ExtLoadLua(M.key,"Act_042_YGHHLItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_042_YGHHLPanel")
...
local gift_data
M.type = 21
M.item_key = "prop_fish_drop_act_0" --阳光
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_042_YGHHLPanel.Create(parm.parent,parm.backcall)
        end 
    else
...
function M.Refresh()
    ...
    Event.Brocast("model_yghhl_data_change_msg")--刷新panel
end
...
function M.QueryGiftData()
    ...
    if gift_data and CheckGiftDataFinish() and (os.time() - this.m_data.time < 5) then
        Event.Brocast("model_yghhl_data_change_msg")
    else
        M.query_data()
    end
end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data then
        if data.result == 0 then
            M.query_data()
            Event.Brocast("yghhl_sw_kfPanel_msg",data.id)
        else
