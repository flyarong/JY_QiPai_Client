...
Act_040_XQDHPanel = basefunc.class()
local C = Act_040_XQDHPanel
C.name = "Act_040_XQDHPanel"
local M = Act_040_XQDHManager
C.instance = nil

local help_info1 = {
    "1.活动时间：12月1日7:30~12月7日23:59:59",
    "2.活动期间街机小游戏中可获得雪球道具",
    "3.活动结束后所有未使用的雪球道具将全部清除，请及时兑换",
...
function C:MakeLister()
	...
	self.lister["model_xqdh_data_change_msg"] = basefunc.handler(self,self.MyRefresh)
	self.lister["xqdh_toggle_set_true_msg"] = basefunc.handler(self,self.on_xqdh_toggle_set_true_msg)
	self.lister["xqdh_toggle_set_false_msg"] = basefunc.handler(self,self.on_xqdh_toggle_set_false_msg)
end
...
function C:MyExit()
	if self.xqdh_timer then
		self.xqdh_timer:Stop()
		self.xqdh_timer = nil
	end
...
function C:CreateItemPrefab()
	...
	for i=1,#self.cur_data do
		local pre = Act_040_XQDHItemBase.Create(self.Content.transform,self.cur_data[i])
		if pre then
...
function C:on_xqdh_toggle_set_true_msg()
...
function C:on_xqdh_toggle_set_false_msg()
	if self.Toggle_tge.isOn then
		...
		et = et + 1
		PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."xqdh",et)
	end
