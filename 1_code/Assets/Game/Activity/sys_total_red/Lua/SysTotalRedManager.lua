...
SysTotalRedManager = {}
local M = SysTotalRedManager
M.key = "sys_total_red"
GameButtonManager.ExtLoadLua(M.key, "TotalRedPrefab")
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return TotalRedPrefab.Create(parm.transform, {game_id = parm.game_id})
    elseif parm.goto_scene_parm == "panel_close" then
        return TotalRedPrefab.Exit()
    else
