...
-- M.shopid1 = 10758
-- M.shopid2 = 10759
Act_056_WYFLManager.type = "wykl_wyfl"
Act_056_WYFLManager.award_config = GameButtonManager.ExtLoadLua(M.key,"act_056_award_config")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLEnterPrefab")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLBeforeBuyPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLAfterBuyPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLLotteryPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_056_WYFLEnterPrefab_InFLPanel")
local this
local lister
...
function M.IsActive()
    ...
    local wyfl_time = PlayerPrefs.GetInt(MainModel.UserInfo.user_id .. M.type)
    if wyfl_time and wyfl_time == 0 and os.time() >= 1620057599 then
        return false--前7天内没买过的就不显示入口
    end


    dump(wyfl_time, "<color=white>wyfl_time</color>")
    if wyfl_time and wyfl_time ~= 0 and os.time() - (wyfl_time - (wyfl_time + 8*3600)%86400) >= 15552000 then
        return false--从购买礼包起,180天后,活动消失
    end

    local _permission_key = "actp_buy_gift_bag_class_wyfl_no_wqp_nor"
    -- 对应权限的key
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_056_WYFLPanel.Create(parm.parent,parm.backcall)
        end 
    elseif parm.goto_scene_parm == "enter" then
        if M.CheckIsShow() and os.time() <= 1620057599  then
            return Act_056_WYFLEnterPrefab.Create(parm.parent, parm.cfg)
        end 
    elseif parm.goto_scene_parm == "jyfl_enter" then
        if M.CheckIsShow() and os.time() >= 1620057599 then
            return Act_056_WYFLEnterPrefab_InFLPanel.Create(parm.parent)
        end
    else
    ...
    if b then
        ...
        _time_IsAfter7Day = Timer.New(function ()
            ...
            if os.time() >= 1591027199 then
                ...
                --Event.Brocast("model_wyfl_EnterPrefab_move")
...
function M.QueryUnrealyData()
    Network.SendRequest("get_one_welfare_activity_false_lottery_data",{act_type = Act_056_WYFLManager.type})
end
...
function M.query_data()
    Network.SendRequest("query_player_welfare_activity_base_info",{act_type = Act_056_WYFLManager.type})   
end
...
function M.on_get_one_welfare_activity_false_lottery_data_response(_,data)
    ...
    if data.result == 0 then
        ...
        Event.Brocast("model_wyfl_unrealy_change_msg")--刷新假数据
    end
end
...
function M.on_welfare_activity_lottery_response(_,data)
    ...
    if data.result == 0 then
        ...
        this.m_data.lottery_num = data.lottery_num      --抽奖次数
        Event.Brocast("model_wyfl_lottery_change_msg")
    end
end
--login_benefits_receive_award
function M.on_welfare_activity_receive_award_response(_,data)
    dump(data,"<color=blue>on_welfare_activity_receive_award</color>")
    if data.result == 0 then
        this.m_data.is_receive = 1

        Network.SendRequest("query_player_welfare_activity_base_info",{act_type = Act_056_WYFLManager.type})
        Event.Brocast("model_wyfl_receive_award_change_msg")--播放领取后抽奖次数增加的的特效
    end
end
