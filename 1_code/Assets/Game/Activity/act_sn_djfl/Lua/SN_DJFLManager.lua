...
SN_DJFLManager = basefunc.class()
local M = SN_DJFLManager
M.key = "act_sn_djfl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_mouse_duiju_config")
local lister
local task_ids
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21102", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
