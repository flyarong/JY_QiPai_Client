...
XXLXRHB_JYFLEnterPrefab = basefunc.class()
local C = XXLXRHB_JYFLEnterPrefab
...
function C:MakeLister()
    ...
    self.lister["model_xxl_xrhb_jyfl_msg"] = basefunc.handler(self, self.on_model_xxl_xrhb_jyfl_msg)
end
...
function C:ctor(parent)
	...
	local obj = newObject("XXLXRHB_JYFLEnterPrefab", parent)
...
function C:MyRefresh()
	self.task_id = XXLXRHBManager.GetJYFLShowId()
	if not self.task_id then
		self.gameObject:SetActive(false)
	else
		self.task_data = XXLXRHBManager.GetTaskData(self.task_id)
...
function C:on_global_hint_state_change_msg(parm)
	if parm.gotoui == XXLXRHBManager.key then
		self:MyRefresh()
	end
end

function C:on_model_xxl_xrhb_jyfl_msg()
	self:MyRefresh()
end
