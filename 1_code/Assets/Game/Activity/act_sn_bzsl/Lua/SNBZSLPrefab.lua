...
SNBZSLPrefab = basefunc.class()
local C = SNBZSLPrefab
C.name = "SNBZSLPrefab"
...
function C:ctor(parent_transform, ui_pos, call, panelSelf)
	...
	local obj = newObject("snbzsl_prefab", parent_transform)
	local tran = obj.transform
	self.transform = tran
	self.gameObject = obj
	LuaHelper.GeneratingVar(self.transform, self)
	self.bianpao_anim = self.bianpao:GetComponent("Animator")
	self:MakeLister()
	self:AddMsgListener()

	change_renderer(self.bianpao, 5, true)
	self:InitUI()
end
...
function C:MyRefresh()
	...
	if self.m_data then
		if self.index then
			...
			self.bianpao.gameObject:SetActive(false)
			...
			self.bianpao.gameObject:SetActive(true)
			self.bianpao_anim:Play("snbzsl_prefab_bianpao_nor", -1, 0)
		end
	else
		self.zm.gameObject:SetActive(false)
		self.bianpao.gameObject:SetActive(true)
...
function C:RunOpenAnim(call)
	self.bianpao_anim:Play("snbzsl_prefab_bianpao", -1, 0)
    ExtendSoundManager.PlaySound(audio_config.game.bzsl_firecracker.audio_name)
