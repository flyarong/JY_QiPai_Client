...
M.key = "act_043_sdlw"
Act_043_SDLWManager.config = GameButtonManager.ExtLoadLua(M.key,"act_043_sdlw_config")
GameButtonManager.ExtLoadLua(M.key,"Act_043_SDLWItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_043_SDLWPanel")
...
local gift_data
M.type = 22
M.item_key = "prop_fish_drop_act_0" --袜子
--money "106306","prop_fish_drop_act_0",100000
local limit_icons = 
{
    "xqdh_icon_xl1_activity_act_043_sdlw",
    "xqdh_icon_xl2_activity_act_043_sdlw",
    "xqdh_icon_xl3_activity_act_043_sdlw"
}
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_043_SDLWPanel.Create(parm.parent,parm.backcall)
        end 
    else
...
function M.Refresh()
    ...
    Event.Brocast("model_"..M.key.."_data_change_msg")--刷新panel
end
...
function M.QueryGiftData()
    ...
    if gift_data and CheckGiftDataFinish() and (os.time() - this.m_data.time < 5) then
        Event.Brocast("model_"..M.key.."_data_change_msg")
    else
        M.query_data()
    end
end
...
function M.GetCurData()
    ...
    for i=1,#M.config.Info do
        ...
        if M.config.Info[i].tips_tit then
            _cur_data[i].tips_tit = M.config.Info[i].tips_tit--奖励描述的标题
        end

        if M.config.Info[i].tips_desc then
            _cur_data[i].tips_desc = M.config.Info[i].tips_desc--奖励描述
        end
    end
    return _cur_data
end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data then
        if data.result == 0 then
            M.query_data()
            Event.Brocast(M.key.."_sw_kfPanel_msg",data.id)
        else
