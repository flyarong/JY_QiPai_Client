...
Act_043_SDLWItemBase = basefunc.class()
local C = Act_043_SDLWItemBase
C.name = "Act_043_SDLWItemBase"
...
function C:MakeLister()
	...
	self.lister[M.key.."_sw_kfPanel_msg"] = basefunc.handler(self,self.on_sw_kfPanel_msg)
end
...
function C:on_enough_BuyClick()
	if os.time() >= PlayerPrefs.GetInt(MainModel.UserInfo.user_id..M.key) then
		Event.Brocast(M.key.."_toggle_set_true_msg")
		HintPanel.Create(2,"是否兑换"..self.data.award_name,function ()
			Event.Brocast(M.key.."_toggle_set_false_msg")
...
function C:on_sw_kfPanel_msg(id)
