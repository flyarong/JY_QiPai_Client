...
Act_013LGFLManager = basefunc.class()
local M = Act_013LGFLManager
M.key = "act_013_lgfl"
local config1 = GameButtonManager.ExtLoadLua(M.key, "act_013_lgfl_config1")
local config2 = GameButtonManager.ExtLoadLua(M.key, "act_013_lgfl_config2")
local lister
local task_ids
local task_ids1
local task_ids2
M.now_level = 0
local permisstions = {
    "actp_own_task_p_rabate_gift1",
    "actp_own_task_p_rabate_gift2",
}
...
function M.IsActive()
    ...
    -- 对应权限的key
    dump(M.GetNowPerMiss(),"<color=red>连购返利权限</color>")
