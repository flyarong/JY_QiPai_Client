...
Act_026_ZQHHLManager = {}
local M = Act_026_ZQHHLManager
M.key = "act_026_zqhhl"
Act_026_ZQHHLManager.config = GameButtonManager.ExtLoadLua(M.key,"act_026_zqhhl_config")
GameButtonManager.ExtLoadLua(M.key,"Act_026_ZQHHLItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_026_ZQHHLPanel")
local this
local lister
local gift_ids
local task_map_ids
...
function M.IsActive()
    -- 活动的开始与结束时间
    local e_time = 1598889600
    local s_time = 1598311800
    if (e_time and os.time() > e_time) or (s_time and os.time() < s_time) then
        return false
    end

    -- 对应权限的key
    local _permission_key = "actp_lmqx_zqhhl_vip"
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_026_ZQHHLPanel.Create(parm.parent,parm.backcall)
        end 
    else
...
function M.Refresh()
    ...
    Event.Brocast("model_zqhhl_data_change_msg")--刷新panel
end
...
function M.QueryGiftData()
    ...
    if gift_data and CheckGiftDataFinish() and (os.time() - this.m_data.time < 5) then
        Event.Brocast("model_zqhhl_data_change_msg")
    else
        M.query_data()
    end
end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data then
        if data.result == 0 then
            M.query_data()
            Event.Brocast("zqhhl_sw_kfPanel_msg",data.id)
        else
            HintPanel.ErrorMsg(data.result)
        end
    end
