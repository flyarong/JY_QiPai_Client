...
Act_026_ZQHHLPanel = basefunc.class()
local C = Act_026_ZQHHLPanel
C.name = "Act_026_ZQHHLPanel"
local M = Act_026_ZQHHLManager
C.instance = nil

local help_info1 = {
"1.活动时间：8月25日7:30-8月31日23:59:59",
"2.活动结束后爱心道具统一清除，请及时进行兑换",
"3.实物奖励图片仅供参考，请以实际发出的奖励为准",
"4.本活动实物奖励为自主兑换，玩家获得后不能兑换成游戏内其他道具",
}
...
function C:MakeLister()
	...
	self.lister["model_zqhhl_data_change_msg"] = basefunc.handler(self,self.MyRefresh)
	self.lister["zqhhl_toggle_set_true_msg"] = basefunc.handler(self,self.on_zqhhl_toggle_set_true_msg)
	self.lister["zqhhl_toggle_set_false_msg"] = basefunc.handler(self,self.on_zqhhl_toggle_set_false_msg)
end
...
function C:MyExit()
	if self.zqhhl_timer then
		self.zqhhl_timer:Stop()
		self.zqhhl_timer = nil
	end
...
function C:CreateItemPrefab()
	...
	for i=1,#self.cur_data do
		local pre = Act_026_ZQHHLItemBase.Create(self.Content.transform,self.cur_data[i])
		if pre then
...
function C:on_zqhhl_toggle_set_true_msg()
	self.zqhhl_timer = Timer.New(function ()
		self.Toggle_tge.gameObject:SetActive(true)
	end,0.1,1,true)
	self.zqhhl_timer:Start()
end

function C:on_zqhhl_toggle_set_false_msg()
	if self.Toggle_tge.isOn then
		...
		et = et + 1
		PlayerPrefs.SetInt(MainModel.UserInfo.user_id.."zqhhl",et)
	end
