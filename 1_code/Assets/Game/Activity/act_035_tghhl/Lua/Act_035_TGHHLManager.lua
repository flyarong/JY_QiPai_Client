-- 创建时间:2020-07-27
-- Act_035_TGHHLManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_035_TGHHLManager = {}
local M = Act_035_TGHHLManager
M.key = "act_035_tghhl"
local prefab
GameButtonManager.ExtLoadLua(M.key,"Act_035_TGHHLPanel")
GameButtonManager.ExtLoadLua(M.key,"Act_035_TGHHLPrefab")
--"♠ : 1","♥ : 2","♣ : 3","♦ : 4"
M.parm = {
    [1] = "prop_fish_drop_act_1",
    [2] = "prop_fish_drop_act_2",
    [3] = "prop_fish_drop_act_3",
    [4] = "prop_fish_drop_act_0",
}
M.parm_img = {
    [1] = "tghhl_icon_2",
    [2] = "tghhl_icon_3",
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_035_TGHHLPanel.Create(parm.parent)
        end
...
function M.EnterScene()
	...
	if MainModel.myLocation == "game_Fishing" and is_nor then
        prefab = Act_035_TGHHLPrefab.Create()
...
function M.IsCanGetAward()
    ...
    if GameItemModel.GetItemCount(M.parm[4]) >= 25 then
        return true
    end
    return false
end
