-- 创建时间:2020-04-28
-- Act_011_XXLPHBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_011_XXLPHBManager = {}
local M = Act_011_XXLPHBManager
M.key = "act_011_xxlphb"
GameButtonManager.ExtLoadLua(M.key, "Act_011_XXLPHBPanel")
local this
local lister
...
local best_rank = 1
M.base_types = {
    "20_4_xiaoxiaole_danbi_yingjin_rank",  --单笔赢金
    "20_4_xiaoxiaole_danbi_rate_rank", --单笔倍数
}
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_011_XXLPHBPanel.Create(parm.parent)
        end
    end 
...
function M.ShowNoticePrefab()
    ...
    if MainModel.myLocation == "game_EliminateSH" 
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_011_XXKPHBPrefab",btn_gameObject.transform)
...
function M.query_rank_base_info_response(_,data)
    ...
    if data and data.result == 0 then
        ...
        Event.Brocast("act_011_xxlphb_base_info_get",{rank_type = data.rank_type})
    end
end
