...
function C:ctor(parent,isbig)
	...
	self.bg.gameObject:SetActive(not(not isbig))
	if not isbig then
		self.transform.localPosition = Vector3.New(0, -32, 0)
	end
	self:MakeLister()
	self:AddMsgListener()
	self:InitUI()
...
function C:MyRefresh()
	...
	if data.total_remain_num_2 >= 1 and (Act_004JIKAManager and not Act_004JIKAManager.getIsBuy()) then
