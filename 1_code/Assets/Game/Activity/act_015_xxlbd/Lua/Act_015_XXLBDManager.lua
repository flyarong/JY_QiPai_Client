-- 创建时间:2020-05-25
-- Act_015_XXLBDManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_015_XXLBDManager = {}
local M = Act_015_XXLBDManager
M.key = "act_015_xxlbd"
GameButtonManager.ExtLoadLua(M.key, "Act_015_XXLBDPanel")
GameButtonManager.ExtLoadLua(M.key, "Act_015_XXLBDEnterPrefab")
local this
local lister
local notice_timer

M.base_types = {
    "xiaoxiaole_20_7_rate_rank",
}
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_015_XXLBDPanel.Create(parm.parent)
        end
    elseif parm.goto_scene_parm == "enter" then
        if M.IsActive() then
            return Act_015_XXLBDEnterPrefab.Create(parm.parent, parm.cfg)
        end 
    end 
...
function M.ShowNoticePrefab()
    ...
    if MainModel.myLocation == "game_Eliminate" or MainModel.myLocation == "game_EliminateSH" or MainModel.myLocation == "game_EliminateCS" then
        M.StopNoticeTime()
        if btn_gameObject and IsEquals(btn_gameObject) and IsEquals(notice_prefab) then
            local time_index = 0
            local space = 60
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("act_015_xxlbdprefab_activity_act_015_xxlbd",btn_gameObject.transform)
