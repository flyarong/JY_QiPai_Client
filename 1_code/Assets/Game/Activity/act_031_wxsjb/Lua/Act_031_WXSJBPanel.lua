...
Act_031_WXSJBPanel = basefunc.class()
local C = Act_031_WXSJBPanel
C.name = "Act_031_WXSJBPanel"
...
local DESCRIBE_TEXT = {
	...
	"2.活动期间，街机捕鱼和所有消消乐游戏都有机会获得五星道具，收集五星道具可参与排行榜",
	"3.获得五星道具越多，排名越靠前，数量相同时，上榜时间越早排名越靠前",
	"4.游戏档次越高，上榜机会越大",
...
function C:MakeLister()
	self.lister = {}
	self.lister["act_025_xxlbd_base_info_get"] = basefunc.handler(self,self.on_act_025_xxlbd_base_info_get)
...
function C:onMyInfoGet()
	...
	if data and data.result ==  0 and IsEquals(self.gameObject)  then
		...
		if not json_data then
			...
			elseif json_data.source_type == "xiaoxiaole_xiyou_award_rate" then
				txt = "西游消消乐"
				color1 = type_color["xyxxl"]
				color2 = type_color["xyxxl_outline"]
			end
		...
		elseif data.rank < 4 then
			...
			self["my"..curr_index.."_rank2_txt"].gameObject:SetActive(false)
			self["my"..curr_index.."_ranking_img"].sprite=GetTexture("localpop_icon_ranking")
			self["my"..curr_index.."_award_txt"].text = Award[data.rank] or "--"	
		end  
	end 
end
...
function C:onInfoGet(data)
	...
	if data and data.result ==0 and IsEquals(self.gameObject) then 
		for i = 1, #data.rank_data do
			...
			self["I"..curr_index.."_goto_btn"].gameObject:SetActive(false)
			if data.rank_data[i].rank < 4 then
				...
				self["I"..curr_index.."_goto_btn"].gameObject:SetActive(true)
				self["I"..curr_index.."_goto_btn"].onClick:AddListener(
					function()
						ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
						GameManager.CommonGotoScence({gotoui = "game_MiniGame"})
					end
				)
				--//
				if json_data then
					if json_data.source_type == "xiaoxiaole_award_rate" then
						self.I1_goto_sgxxl_btn.gameObject:SetActive(true)
						self.I1_goto_sgxxl_btn.onClick:AddListener(function ()
							ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
							GameManager.CommonGotoScence({gotoui = "game_Eliminate"})
						end)
					elseif json_data.source_type == "xiaoxiaole_shuihu_award_rate" then
						self.I1_goto_shxxl_btn.gameObject:SetActive(true)
						self.I1_goto_shxxl_btn.onClick:AddListener(function ()
							ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
							GameManager.CommonGotoScence({gotoui = "game_EliminateSH"})
						end)
					elseif json_data.source_type == "xiaoxiaole_caishen_award_rate" then
						self.I1_goto_csxxl_btn.gameObject:SetActive(true)
						self.I1_goto_csxxl_btn.onClick:AddListener(function ()
							ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
			...
			if data.rank_data[i].rank >= 1 and data.rank_data[i].rank <= 20 then
				...
				if data.rank_data[i].rank%2 == 0 then
					...
					img.sprite = GetTexture("bzzy_bg_bzzy3")
				else	
					img.sprite = GetTexture("bzzy_bg_bzzy1")
				end	
...
function C:on_act_025_xxlbd_base_info_get()
	dump("<color>+++++++++++++++++++++++++++++++</color>")
	self:onMyInfoGet()
end
