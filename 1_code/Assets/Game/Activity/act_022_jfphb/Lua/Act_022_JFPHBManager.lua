-- 创建时间:2020-05-25
-- Act_022_JFPHBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_022_JFPHBManager = {}
local M = Act_022_JFPHBManager
M.key = "act_022_jfphb"
GameButtonManager.ExtLoadLua(M.key, "Act_022_JFPHBPanel")
GameButtonManager.ExtLoadLua(M.key, "Act_022_JFPHBEnterPrefab")
local this
local lister
local notice_timer

M.base_types = {
    "jifenzhengba_022_rank",
}
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_022_JFPHBPanel.Create(parm.parent)
        end
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_015_XXLBDPrefab",btn_gameObject.transform)
