-- 创建时间:2020-10-26
-- Act_037_LHSJBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_037_LHSJBManager = {}
local M = Act_037_LHSJBManager
M.key = "act_037_lhsjb"
GameButtonManager.ExtLoadLua(M.key, "Act_037_LHSJBPanel")
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_037_LHSJBPanel.Create(parm.parent)
    else
...
function M.query_rank_base_info_response(_,data)
    if data and data.result == 0 then
        ...
        Event.Brocast("act_037_lhsjb_base_info_get",{rank_type = data.rank_type})
    end
end
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_037_LHSJBNoticePrefab",btn_gameObject.transform)
