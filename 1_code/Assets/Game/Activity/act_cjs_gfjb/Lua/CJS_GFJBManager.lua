...
CJS_GFJBManager = basefunc.class()
local M = CJS_GFJBManager
M.key = "act_cjs_gfjb"
GameButtonManager.ExtLoadLua(M.key, "CJS_GFJBListPanel")
GameButtonManager.ExtLoadLua(M.key, "CJS_GFJBHelpPanel")
local config_1 = GameButtonManager.ExtLoadLua(M.key, "activity_task_oldyear_partition_config_1")
local config_2 = GameButtonManager.ExtLoadLua(M.key, "activity_task_oldyear_partition_config_2")
local config_3 = GameButtonManager.ExtLoadLua(M.key, "activity_task_oldyear_partition_config_3")
local config_4 = GameButtonManager.ExtLoadLua(M.key, "activity_task_oldyear_partition_config_4")
local configs = {config_1,config_2,config_3,config_4}
--完成任务后瓜分
local gf_tasks = {21086,21091,21096,21101}
local curr_level = 1
GameButtonManager.ExtLoadLua(M.key, "CJS_GFJBPrefab")
local lister
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        ...
        dump({cfg = cfg , curr_l = curr_l},"<color=red>------瓜分金币----</color>")
        if curr_l then
            cfg = configs[curr_l]
...
function M.GetHintState(parm)
    if parm and parm.gotoui == M.key then
        ...
        if M.IsAwardCanGet() or M.IsCanGF() then
...
function M.level1()
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_cjs_2020_task1", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
...
function M.level2()
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_cjs_2020_task2", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
...
function M.level3()
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_cjs_2020_task3", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
...
function M.level4()
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_cjs_2020_task4", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
