...
M.key = "act_051_nslw"
Act_051_NSLWManager.config = GameButtonManager.ExtLoadLua(M.key,"act_055_nslw_config")
GameButtonManager.ExtLoadLua(M.key,"Act_051_NSLWItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_051_NSLWPanel")
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_051_NSLWPanel.Create(parm.parent,parm.backcall)
        end 
    else
...
function M.Refresh()
    ...
    Event.Brocast("model_ydlw_data_change_msg")--刷新panel
end
...
function M.QueryGiftData()
    ...
    if gift_data and CheckGiftDataFinish() and (os.time() - this.m_data.time < 5) then
        Event.Brocast("model_ydlw_data_change_msg")
    else
        M.query_data()
    end
end
...
function M.on_activity_exchange_response(_,data)
    ...
    if data then
        if data.result == 0 then
            M.query_data()
            Event.Brocast("ydlw_sw_kfPanel_msg",data.id)
        else
