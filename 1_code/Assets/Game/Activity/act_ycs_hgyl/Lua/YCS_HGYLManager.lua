...
YCS_HGYLManager = basefunc.class()
local M = YCS_HGYLManager
M.key = "act_ycs_hgyl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_caishen_regression_config")
local lister
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21122", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
