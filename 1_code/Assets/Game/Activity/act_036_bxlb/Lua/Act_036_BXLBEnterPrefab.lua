...
Act_036_BXLBEnterPrefab = basefunc.class()
local C = Act_036_BXLBEnterPrefab
C.name = "Act_036_BXLBEnterPrefab"
...
function C:ctor(parent)
    ...
    local obj = newObject("Act_036_BXLBEnterPrefab", parent)
...
function C:OnEnterClick()
    dump(M.level,"M.level")
    dump(M.config,"M.config")

    local b = GameComGiftT4Panel.Create(nil, nil, M.config[M.level], nil, "Act_036_BXLBPanel")
    --local b = GameComGiftT4Panel.Create(nil, nil, M.config[2], nil, "Act_036_BXLBPanel")

    for i = 1, 3 do
        local ui = {}
        LuaHelper.GeneratingVar(b["gift_rect" .. i], ui)
        PointerEventListener.Get(ui.icon3_img.gameObject).onDown = function()
            local bx_show = "宝箱开启时间：11月10日7:30~11月16日23:59:59\n有机会获得大量鲸币，话费碎片和充值优惠券\n11月16日23:59:59过后，未使用的宝箱统一清除。"
