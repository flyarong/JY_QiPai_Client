...
NmhgylManager = basefunc.class()
local M = NmhgylManager
M.key = "act_nm_hgyl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_yearend_regression_config")
local lister
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_return_activity", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
