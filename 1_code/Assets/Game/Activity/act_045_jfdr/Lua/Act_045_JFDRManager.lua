...
Act_045_JFDRManager = {}
local M = Act_045_JFDRManager
M.key = "act_045_jfdr"
GameButtonManager.ExtLoadLua(M.key, "Act_045_JFDRPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "yclx_045_jfdr_rank"
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_045_JFDRPanel.Create(parm.parent, parm.backcall)
        end
    end
end
-- 活动的提示状态
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=14>积分达人rank_base_info-----</size></color>")
...
function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=14>积分达人rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
...
function M.on_year_btn_created(data)
    if data and data.enterSelf and false then
        btn_gameObject = data.enterSelf.gameObject
        notice_prefab = newObject("Act_045_JFDRPrefab", btn_gameObject.transform)
...
function M.ShowNoticePrefab()
    ...
    if M.GetBestRank() > 20 or M.GetBestRank() < 2 or true then
        return
    end
