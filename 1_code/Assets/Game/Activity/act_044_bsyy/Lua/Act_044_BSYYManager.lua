...
Act_044_BSYYManager = {}
local M = Act_044_BSYYManager
M.key = "act_044_bsyy"
GameButtonManager.ExtLoadLua(M.key, "Act_044_BSYYPanel") 
GameButtonManager.ExtLoadLua(M.key, "Act_044_BSYYPanel_Out") 
local lister
local m_data
local activity_id_year = 19
local activity_id_game = 19
local activity_ID_game = 19
local is_yy = false
local end_time = 1612011600  
local start_time = 1611617400  
local match_start_time = 1612011600 
local match_day_time = 1611936000--比赛当天零点,比如比赛当天是5月1日21:00点.那么这个值就是5月1日00:00点
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_044_BSYYPanel.Create(parm.parent)
    elseif parm.goto_scene_parm == "panel_out" then
        return Act_044_BSYYPanel_Out.Create(parm.parent)
