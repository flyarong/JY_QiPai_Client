...
Act_040_XQSJBManager = {}
local M = Act_040_XQSJBManager
M.key = "act_040_xqsjb"
GameButtonManager.ExtLoadLua(M.key, "Act_040_XQSJBPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "xqdz_040_xqsjb_rank"
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_040_XQSJBPanel.Create(parm.parent, parm.backcall)
        end
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=14>雪球收集榜rank_base_info-----</size></color>")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        M.InitUserData(data)
        Event.Brocast("act_040_xqsjb_rank_base_info_get",{rank_type = data.rank_type})
    end
end

function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=14>雪球收集榜rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
        Event.Brocast("act_040_xqsjb_rank_info_get",{rank_type = data.rank_type})
    end
end
...
function M.on_year_btn_created(data)
    if  data and data.enterSelf then
        ...
        notice_prefab=newObject("act_040_xqsjbprefab_activity_act_040_xqsjb",btn_gameObject.transform)
