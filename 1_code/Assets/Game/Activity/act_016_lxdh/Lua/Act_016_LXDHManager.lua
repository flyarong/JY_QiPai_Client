...
M.key = "act_016_lxdh"
Act_016_LXDHManager.config = GameButtonManager.ExtLoadLua(M.key,"act_016_lxdh_config")
GameButtonManager.ExtLoadLua(M.key,"Act_016_LXDHItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_016_LXDHPanel")
local this
local lister
local gift_ids
local task_map_ids
...
function M.IsActive()
    ...
    -- 对应权限的key
    local _permission_key = "actp_own_task_longxiaduihuan"
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_016_LXDHPanel.Create(parm.parent,parm.backcall)
        end 
    else
