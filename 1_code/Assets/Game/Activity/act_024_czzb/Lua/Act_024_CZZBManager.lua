...
Act_024_CZZBManager = {}
local M = Act_024_CZZBManager
M.key = "act_024_czzb"
GameButtonManager.ExtLoadLua(M.key, "Act_024_CZZBPHBPanel")
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_024_CZZBPHBPanel.Create(parm.parent)
        end
    end 
...
function M.query_rank_base_info_response(_,data)
    if data and data.result == 0 then
        ...
        Event.Brocast("act_024_base_info_get",{rank_type = data.rank_type})
    end
end
