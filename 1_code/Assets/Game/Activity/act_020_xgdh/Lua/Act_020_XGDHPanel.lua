...
Act_020_XGDHPanel = basefunc.class()
local C = Act_020_XGDHPanel
C.name = "Act_020_XGDHPanel"
local M = Act_020_XGDHManager
C.instance = nil

local help_info1 = {
"1.活动时间：7月14日7:30-7月21日6:30",
"2.活动结束后西瓜道具统一清除，请及时进行兑换",
...
function C:CreateItemPrefab()
	...
	for i=1,#self.cur_data do
		local pre = Act_020_XGDHItemBase.Create(self.Content.transform,self.cur_data[i])
		if pre then
