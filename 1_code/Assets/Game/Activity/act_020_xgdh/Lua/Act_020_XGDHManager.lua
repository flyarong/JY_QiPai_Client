...
Act_020_XGDHManager = {}
local M = Act_020_XGDHManager
M.key = "act_020_xgdh"
Act_020_XGDHManager.config = GameButtonManager.ExtLoadLua(M.key,"act_020_xgdh_config")
GameButtonManager.ExtLoadLua(M.key,"Act_020_XGDHItemBase")
GameButtonManager.ExtLoadLua(M.key,"Act_020_XGDHPanel")
local this
local lister
local gift_ids
local task_map_ids
...
function M.IsActive()
    ...
    -- 对应权限的key
    local _permission_key = "actp_own_task_watermelon_convert"
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_020_XGDHPanel.Create(parm.parent,parm.backcall)
        end 
    else
