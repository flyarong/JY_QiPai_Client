...
Act_032_XXLZBManager = {}
local M = Act_032_XXLZBManager
M.key = "act_032_xxlzb"
GameButtonManager.ExtLoadLua(M.key, "Act_032_XXLZBPHBPanel")
local this
local lister
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_032_XXLZBPHBPanel.Create(parm.parent)
        end
    end 
...
function M.query_rank_base_info_response(_,data)
    if data and data.result == 0 then
        ...
        Event.Brocast("act_032_base_info_get",{rank_type = data.rank_type})
    end
end
