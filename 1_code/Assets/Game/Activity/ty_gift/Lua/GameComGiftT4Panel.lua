...
function C.Create(parent, backcall, config,isMini,ExtPrefabName)
	dump(config,"<color=red>configconfigconfigconfig</color>")
	return C.New(parent, backcall, config,isMini,ExtPrefabName)
end
...
function C:ctor(parent, backcall, config,isMini,ExtPrefabName)
	...
	self.config = config
	local Cname  = isMini and "GameComGiftT4MiniPanel" or (ExtPrefabName or C.name)
