...
CJS_ZDFLManager = basefunc.class()
local M = CJS_ZDFLManager
M.key = "act_cjs_zdfl"
local config = GameButtonManager.ExtLoadLua(M.key, "activity_task_oldyear_boom_config")
local lister
...
function M.CheckIsShowInActivity(parm)
    local a,b = GameButtonManager.RunFun({gotoui="sys_qx", _permission_key="actp_own_task_21116", is_on_hint = true}, "CheckCondition")
    if a and not b then
        return false
    end
    return true
