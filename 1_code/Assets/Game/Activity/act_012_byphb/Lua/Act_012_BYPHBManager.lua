-- 创建时间:2020-04-28
-- Act_012_BYPHBManager 管理器

local basefunc = require "Game/Common/basefunc"
Act_012_BYPHBManager = {}
local M = Act_012_BYPHBManager
M.key = "act_012_byphb"
GameButtonManager.ExtLoadLua(M.key, "Act_012_BYPHBPanel")
local this
local lister
...
local best_rank = 1
M.base_types = {
    "20_5_buyu_yingjin_rank",  --单笔赢金
    "20_5_buyu_rate_rank", --单笔倍数
}
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        if M.IsActive() then
            return Act_012_BYPHBPanel.Create(parm.parent)
        end
    end 
...
function M.on_year_btn_created(data)
    if data and data.enterSelf then
        ...
        notice_prefab = newObject("Act_012_BYPHBPrefab",btn_gameObject.transform)
