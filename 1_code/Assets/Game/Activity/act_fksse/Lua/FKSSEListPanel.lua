...
FKSSEListPanel = basefunc.class()
local C = FKSSEListPanel
C.name = "FKSSEListPanel"
local Page_Index  =  1
function C.Create()
	return C.New()
end
...
function C:ctor()
	...
	self:AddMsgListener()
	self:InitUI()
	Network.SendRequest("common_lottery_query_award_logs",{page_index = Page_Index,lottery_type = "box_exchange_3"})
end
