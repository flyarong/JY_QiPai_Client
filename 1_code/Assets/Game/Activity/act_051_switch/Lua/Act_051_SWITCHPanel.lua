...
Act_051_SWITCHPanel = basefunc.class()
local C = Act_051_SWITCHPanel
C.name = "Act_051_SWITCHPanel"
local M = Act_051_SWITCHManager

local DESCRIBE_TEXT = {
	"1.活动时间：8月11日7:30~8月17日23:59:59",
	"2.活动期间鱼场中会出现章鱼BOSS，击杀后获得海盗船长帽，部分小鱼击杀后获得黑红梅方道具，炮倍率越高，花色掉落数量越多。",
	"3.积分可用于参与船长争霸，第1名1000福卡。",
	"4.活动结束后所有道具全部清除，请您及时兑换奖励。",
}
...
function C:ctor(parent,backcall)
	...
	CommonTimeManager.GetCutDownTimer(1615219200,self.cut_txt)
	if next(self.config) then
		-- if Act_051_BZZBManager.GetBestRank() <= 20 then
...
function C:on_had_switch_btn(parm)
	if parm then
		...
		if parm.gotoui == "act_051_bzdh" then
...
function C:WorkBtn(temp_ui,manager_str)
	if manager_str == "act_051_nslw" then
		dump(temp_ui)
		temp_ui.main_img.sprite = GetTexture("nvlw2_bg")
		temp_ui.mask_img.sprite  = GetTexture("nvlw1_bg")			
	else
		...
		temp_ui.main_img.sprite  = GetTexture("nvshz2_bg")
		temp_ui.mask_img.sprite  = GetTexture("nvshz1_bg")
	end
end
...
function C:LoadGiftBtn()
	local _parm = {}
	GameManager.GotoUI({gotoui="act_ty_gifts", goto_scene_parm="enter", goto_type = "gift_shlb",parent = self.gift_node.transform})
end
