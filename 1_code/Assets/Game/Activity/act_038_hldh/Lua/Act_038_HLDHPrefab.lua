...
Act_038_HLDHPrefab = basefunc.class()
local C = Act_038_HLDHPrefab
C.name = "Act_038_HLDHPrefab"
...
function C:fishing_pos_change(data)
    if data then
        if data.isShow then
            self.transform.localPosition = Vector3.New(0,277,0)
        else
            self.transform.localPosition = Vector3.New(0,437,0)
        end
    end
