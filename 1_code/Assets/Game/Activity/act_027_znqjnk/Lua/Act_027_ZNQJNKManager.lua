...
Act_027_ZNQJNKManager = {}
local M = Act_027_ZNQJNKManager
M.key = "act_027_znqjnk"
GameButtonManager.ExtLoadLua(M.key, "Act_027_ZNQJNKPanel")
GameButtonManager.ExtLoadLua(M.key, "Act_027_ZNQJNKEnterPrefab")
GameButtonManager.ExtLoadLua(M.key, "Act_027_ZNQJNKJYFLEnterPrefab")
M.config = GameButtonManager.ExtLoadLua(M.key, "act_027_znqjnk_config")
local this
local lister
M.gift_id_998 = 10400
M.gift_id_2498 = 10401
-- buy_time
M.start_time = 1600126200
M.end_time = 1600703999
-- 是否有活动
function M.IsActive()
    -- 活动的开始与结束时间
...
function M.GotoUI(parm)
    if parm.goto_scene_parm == "panel" then
        return Act_027_ZNQJNKPanel.Create(parm.parent,parm.backcall)
    end 
    if parm.goto_scene_parm == "enter" then
        ...
        return Act_027_ZNQJNKEnterPrefab.Create(parm.parent)
    end 
    if parm.goto_scene_parm == "jyfl_enter" then
        if not M.CheckIsShowInJYFL(parm) then return end
        return Act_027_ZNQJNKJYFLEnterPrefab.Create(parm.parent, parm)
    end 
end
-- 活动的提示状态
...
local function MakeLister()
    ...
    lister["query_jinianka_2_anniversary_data_response"] = this.on_query_jinianka_2_anniversary_data_response
    lister["jinianka_2_anniversary_data_change"] = this.jinianka_2_anniversary_data_change    
...
function M.OnLoginResponse(result)
    dump(data,"<color=white>2周年纪念卡</color>")
    if result == 0 then
        Network.SendRequest("query_jinianka_2_anniversary_data",nil)
...
function M.on_query_jinianka_2_anniversary_data_response( _,data )
    dump(data,"<color=green>2周年纪念卡</color>")
    if data and data.result == 0 then
        this.data = data
        M.SetHintState()
    end
    Event.Brocast("model_query_jinianka_2_anniversary_data_response", data)
end
...
function M.jinianka_2_anniversary_data_change(_,data)
    dump(data,"<color=green>2周年纪念卡改变-------------</color>")
    this.data = data
    M.SetHintState()
    Event.Brocast("model_jinianka_2_anniversary_data_change", data)
end
...
function M.EnterScene(  )
    Event.Brocast("act_027_znqjnk_close")
end
