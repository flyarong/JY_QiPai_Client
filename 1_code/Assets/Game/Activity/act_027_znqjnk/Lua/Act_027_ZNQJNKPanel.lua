...
Act_027_ZNQJNKPanel = basefunc.class()
local C = Act_027_ZNQJNKPanel
C.name = "Act_027_ZNQJNKPanel"
local lottery_type = {
	day = 1,
	week = 2,
	month = 3,
...
local Mgr = Act_027_ZNQJNKManager
...
function C:MakeLister()
	self.lister = {}
	self.lister["model_query_jinianka_2_anniversary_data_response"] = basefunc.handler(self,self.on_query_jinianka_2_anniversary_data_response)
	self.lister["model_jinianka_2_anniversary_data_change"] = basefunc.handler(self,self.jinianka_2_anniversary_data_change)
	self.lister["get_jinianka_2_anniversary_award_response"] = basefunc.handler(self,self.on_get_jinianka_2_anniversary_award_response)
	self.lister["AssetsGetPanelConfirmCallback"] = basefunc.handler(self,self.AssetsGetPanelConfirmCallback)
	self.lister["AssetChange"] = basefunc.handler(self, self.OnAssetChange)
	self.lister["act_027_znqjnk_close"] = basefunc.handler(self, self.MyExit)
end

function C:OnAssetChange(data)
	dump(data, "<color=red>----奖励类型-----</color>")
	if data.change_type and data.change_type == "jinianka_2_anniversary_award" then
		self.cur_award = data
	end
end
...
function C:ctor(parent,backcall)
	...
	self:CloseAnimSound()
	Network.SendRequest("query_jinianka_2_anniversary_data")
	self.timer = {}
...
function C:InitUI()
	...
	for k,j in pairs(lottery_type) do
		...
		function ()
			...
			Network.SendRequest("get_jinianka_2_anniversary_award",{award_type = j}) 
		end
	)
	end
	obj = nil
...
function C:AnimStart(selectIndex,award_type)
	if not self.is_during_anim then
		...
		local rota = -360 * 16 - 45 * (selectIndex - 1)
		local seq = DoTweenSequence.Create()
		seq:Append(self.zhizhen[award_type]:DORotate( Vector3.New(0, 0 , rota), 6, DG.Tweening.RotateMode.FastBeyond360):SetEase(DG.Tweening.Ease.InOutCubic))
		seq:OnKill(function ()
			if IsEquals(self.gameObject) then 
				self.zhizhen[award_type].localRotation = Quaternion:SetEuler(0, 0, rota)
				self:CloseAnimSound()
...
function C:on_query_jinianka_2_anniversary_data_response(data)
...
function C:on_get_jinianka_2_anniversary_award_response(_,data)
...
function C:jinianka_2_anniversary_data_change(data)
