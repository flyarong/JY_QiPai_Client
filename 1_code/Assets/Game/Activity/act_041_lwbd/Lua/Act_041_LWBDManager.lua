...
Act_041_LWBDManager = {}
local M = Act_041_LWBDManager
M.key = "act_041_lwbd"
GameButtonManager.ExtLoadLua(M.key, "Act_041_LWBDPanel")
...
local user_data = {}
local rank_data = {}
local _rank_type = "zhshe_041_lwbd_rank"
...
function M.GotoUI(parm)
    ...
    if parm.goto_scene_parm == "panel" then
        if M.CheckIsShow() then
            return Act_041_LWBDPanel.Create(parm.parent, parm.backcall)
        end
...
function M.query_rank_base_info_response(_, data)
    dump(data, "<color=red><size=15>JJCY榜单rank_base_info-----</size></color>")
    if data and data.result == 0 then
        this.m_data.mydata[data.rank_type] = data
        M.InitUserData(data)
        --Event.Brocast("act_040_xqsjb_base_info_get",{rank_type = data.rank_type})
    end
end

function M.query_rank_data_response(_, data)
    -- body
    dump(data, "<color=red><size=15>JJCY榜单rank_data-------</size></color>")
    if data and data.result == 0 then
        M.InitRankData(data)
        --Event.Brocast("act_040_xqsjb_base_info_get",{rank_type = data.rank_type})
    end
...
function M.on_year_btn_created(data)
    if  data and data.enterSelf then
        ...
        notice_prefab=newObject("Act_040_XQSJBPrefab",btn_gameObject.transform)
