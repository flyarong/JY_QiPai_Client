...
function M.OnAssetChange(data)
    Event.Brocast("global_hint_state_change_msg",{gotoui = M.key})
    if data.change_type and (data.change_type == "task_p_031_gqkl_nor" or "task_p_031_gqkl_cpl" == data.change_type) then
        M.PrefabCreator(data.data[1].value)
    end
end

function M.PrefabCreator(value)
    if not GameObject.Find("Canvas/LayerLv50") then return end
    local temp_ui = {}
    local can_auto = true
    local can_click = true
    local obj = newObject("Act_031_GQKLPrefab",GameObject.Find("Canvas/LayerLv50").transform)
...
function M.AddUnShowAward()
    local check_func = function (type)
        if "task_p_031_gqkl_nor" == type or "task_p_031_gqkl_cpl" == type then
            return true
        end
    end
