-- 创建时间:2020-05-12
-- Panel:Act_044_XNFLLotteryPanel
--[[
*      ┌─┐       ┌─┐
*   ┌──┘ ┴───────┘ ┴──┐
*   │                 │
...
Act_044_XNFLLotteryPanel = basefunc.class()
local C = Act_044_XNFLLotteryPanel
C.name = "Act_044_XNFLLotteryPanel"
...
function C:MakeLister()
	...
	self.lister["model_xnfl_unrealy_change_msg"] = basefunc.handler(self,self.on_model_xnfl_unrealy_change_msg)
	self.lister["model_xnfl_lottery_change_msg"] = basefunc.handler(self,self.on_model_xnfl_lottery_change_msg)
...
function C:InitUI()
	self.lottery_btn.onClick:AddListener(function ()
		...
		Network.SendRequest("welfare_activity_lottery",{act_type = M.type})
...
--虚假数据跑马灯
function C:on_model_xnfl_unrealy_change_msg()
	self:PlayPmd(false)
end

function C:PlayPmd(isPlayer)
	local playerName, awardName, delayTime
	if isPlayer then
		playerName = M.GetLotteryPlayerName()
		awardName = M.GetLotteryAwardName()
		delayTime = 5
	else
		playerName = M.GetUnrealyPlayerName()
		awardName = M.GetUnrealyAwardName()
		delayTime = 0.1
	end

	self.Lantern.gameObject:SetActive(true)
	self.Lantern2_txt.text = "恭喜玩家<color=#e7ff62>"..(playerName or "").."</color>抽中了<color=#ffc617>"..(awardName or "").."</color>"
	self.seq = DoTweenSequence.Create()
	self.seq:AppendInterval(delayTime)
...
function C:on_model_xnfl_lottery_change_msg()
	self.times_txt.text = "剩余次数:"..(M.GetLotteryNum() or "0")
