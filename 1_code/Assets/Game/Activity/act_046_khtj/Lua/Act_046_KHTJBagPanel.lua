...
Act_046_KHTJBagPanel = basefunc.class()
local C = Act_046_KHTJBagPanel
C.name = "Act_046_KHTJBagPanel"
...
function C:InitUI()
	...
	for i = 1,#M.config.base do
		local num = GameItemModel.GetItemCount(M.config.base[i].item_key)
		all_num = all_num + num
		if num > 0 then
			...
			temp.item_img.sprite = GetTexture(M.config.base[i].icon)
		end
	end
	self.all_num = all_num
	self.t1_txt.text = all_num
	self.t2_txt.text = all_num * 100
	...
	self.go_btn.onClick:AddListener(function()
		HintPanel.Create(2,"确认全部兑换吗？",function()
			Network.SendRequest("box_all_exchange",{name = "khtj_1_12" })
		end)
	end)
	self:MyRefresh()
end
