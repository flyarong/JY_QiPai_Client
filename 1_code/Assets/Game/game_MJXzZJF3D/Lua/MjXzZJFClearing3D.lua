...
MjXzFKClearing.name = "MjXzZJFClearing3D"
...
function MjXzFKClearing.isGameOverStatus()
	return MjXzFKModel.data.model_status == MjXzFKModel.Model_Status.gameover
end
...
function MjXzFKClearing:AddMsgListener(lister)
...
function MjXzFKClearing:MyRefresh()
...
function MjXzFKClearing:ShowMyself()
	PlayerInfoPanel.Exit()
    GameSpeedyPanel.Hide()
	if self.delayShow then
		self.delayShow:Stop()
...
function MjXzFKClearing:InitRect()
	if MjXzFKModel.data.glory_score_count then
		local v1 = MjXzFKModel.data.glory_score_count
		local v2 = MjXzFKModel.data.glory_score_change
	...
	if MjXzFKModel.data.score_change_list then
		dump(MjXzFKModel.data.score_change_list, "<color=yellow>==============================================>>> MjXzFKModel.data.score_change_list:</color>")
		self:GetMyScoreChange(MjXzFKModel.data.score_change_list)
	else
	...
	for _, data in ipairs(clearData) do
		local i = MjXzFKModel.GetSeatnoToPos (data.seat_num)
		if i == 1 then
	...
	-- local cfg = GameFreeModel.GetGameIDToConfig(MjXzFKModel.data.game_id)
...
function MjXzFKClearing:InitOthers(i, data)
	...
	local base = MjXzFKModel.data.game_players_info[seatno]
	...
	--- 飘的多少倍
	if MjXzFKModel.daPiao then
		local piaoNum = MjXzFKModel.data.playerInfo[seatno].piaoNum
		if piaoNum and (piaoNum == 0 or piaoNum == 1 or piaoNum == 3 or piaoNum == 5 ) then
			
			if piaoNum > 0 then
				rect.piaoIcon.gameObject:SetActive(true)
				--rect.piaoIcon.sprite = GetTexture( MjXzFKModel.piaoIconVec[piaoNum] )
			else
		...
		--- 总共有几个人飘
		--[[local piaoPlayerNum = MjXzFKModel.GetDaPiaoPlayerNum()
		...
		if piaoPlayerNum == 1 then
			rect.daPiaoStakeText.text = string.format("%d人飘底分%d",piaoPlayerNum,2*MjXzFKModel.data.init_stake)
		elseif piaoPlayerNum >= 2 then
			rect.daPiaoStakeText.text = string.format("%d人飘底分%d",piaoPlayerNum,4*MjXzFKModel.data.init_stake)
		end]]
	else
		rect.piaoIcon.gameObject:SetActive(false)
		--rect.daPiaoStakeText.gameObject:SetActive(false)
...
function MjXzFKClearing:CalcMyResult(data)
	local seatno = data.seat_num
	if MjXzFKModel.GetRealPlayerSeat () == seatno then
		self.huindex = 1
		...
		if data.settle_data.settle_type == "hu" then
		    local ui_config = GameFreeModel.GetGameIDToConfig(MjXzFKModel.data.game_id)
        ...
        local a,b = GameButtonManager.RunFun({gotoui="sys_act_operator",game_id = MjXzFKModel.data.game_id}, "CheckShowLS")
		local is_show_ls = a and b
		if isWin and MjXzFKModel.data.ls_count > 1 and is_show_ls then
			self.WinNode:SetActive(false)
			self.count_txt.text = MjXzFKModel.data.ls_count
...
function MjXzFKClearing:CreateDetailItem(col1, col2, col3, col4, isBroke)
...
function MjXzFKClearing.GetGangName(op)
	return (op == "wg" and "刮风" or "下雨")
end

function MjXzFKClearing.GetGangPlusCount(op)
	return (op == "wg" and 1 or 2)
end

function MjXzFKClearing.GetAffectedPlayerPos(mySeat, affected)
	local p = {}
	if #MjXzFKModel.data.settlement_info == 2 then
		p[1] = "对家"
	else
...
function MjXzFKClearing:ShowDetailsAndPlayers(isShow)
...
function MjXzFKClearing:GetPlayerScore(seatno)
	local myScore = 0
	for _, info in ipairs(MjXzFKModel.data.game_players_info) do
...
function MjXzFKClearing:InitBPF(data, bpf)
	dump(data, "<color=yellow>--->>>MjXzFKClearing:InitBPF</color>")
	local yingfengding = MjXzFKModel.data.yingfengding
	...
	--local cfg = GameFreeModel.GetGameIDToConfig(MjXzFKModel.data.game_id)
	--dump(cfg, "<color=yellow>game id:" .. MjXzFKModel.data.game_id .. ", config</color>")
	...
	if yingfengding then
		for i, v in ipairs(yingfengding) do
			if i == data.seat_num and v == 1 then
				msg = "带多少只能赢多少哦！"
				iconImg = "ddz_settlement_icon_yfd_normal_mj_common"
				show = true
				showMsg = true
				break
			end
	...
	if MjXzFKModel.data.game_bankrupt then
        for i, d in ipairs(MjXzFKModel.data.game_bankrupt) do
...
function MjXzFKClearing:OnChangedeskClick()
    MjXzFKModel.HZCheck()
end

-- 准备
function MjXzFKClearing:OnReadyClick()
	MjXzFKModel.ZBCheck()
end

-- 返回
function MjXzFKClearing:OnBackClick()
...
function MjXzFKClearing:OnShareClick()
	if not self:IsCanShare() then return end
	MJSharePrefab.Create(MjXzFKModel.data.game_id, {time=self.gameExitTime, my_score = self.my_score, multi = self.multi,
		paiType=self.pai_type, huType=self.hu_type, pai=self.my_shou_pai, geng=self.geng, huindex=self.huindex})
end
function MjXzFKClearing:on_fg_ready_response_code(result)
	if result == 0 then
		self:Close()
	end
end
function MjXzFKClearing:on_fg_huanzhuo_response_code(result)
	if result == 0 then
		self:Close()
	end
end

function MjXzFKClearing:on_model_fg_gameover_msg()
...
function MjXzFKClearing:TryFixStuck()
...
function MjXzFKClearing:StopFixStuck()
...
function MjXzFKClearing:OnSeeCardBtnClicked()
	self.ShowCard = not self.ShowCard
	self:ShowDetailsAndPlayers(not self.ShowCard)
end

function MjXzFKClearing:OnMissionBtnClicked()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
...
function MjXzFKClearing:CheckShow1YuanGift(call)
	if GameGlobalOnOff.Shop_10_gift_bag ~= nil and GameGlobalOnOff.Shop_10_gift_bag == false then
		if call then call() end
		return
	end

	log("<color=blue>-------------------------------------->>> My seat:" .. MjXzFKModel.data.seat_num .. "</color>")
	local brokeUp = false
	local myScore = self:GetPlayerScore(MjXzFKModel.data.seat_num)
	local gameCfg = GameFreeModel.GetGameIDToConfig(MjXzFKModel.data.game_id)
...
function MjXzFKClearing:GetHuTypeShort(settleData, pg_pai)
...
function MjXzFKClearing:GetMyScoreChange(dataList)
	local index = 1
	local mySeat = MjXzFKModel.data.seat_num
	self.scoreDetails = {}
...
function MjXzFKClearing:InitDetailList(data)
	dump(self.scoreDetails, "<color=green>---------------------------------->>>> score details:</color>")
	dump(data, "<color=green>===============================================>>> settle data:</color>")
	dump(MjXzFKModel.data.settlement_info, "<color=green>===============================================>>> settle info:</color>")

	if self.scoreDetails and #self.scoreDetails > 0 then
		--local gameCfg = GameFreeModel.GetGameIDToConfig(MjXzFKModel.data.game_id)
		local seatno = data.seat_num
		local settleInfo = MjXzFKModel.data.settlement_info
		local details = self.scoreDetails
		local mySeat = MjXzFKModel.data.seat_num
		local piaoNum = 0

		if MjXzFKModel.data.playerInfo then
			for i = 1, #MjXzFKModel.data.playerInfo do
				if MjXzFKModel.data.playerInfo[i].piaoNum and MjXzFKModel.data.playerInfo[i].piaoNum > 0 then
					piaoNum = piaoNum + MjXzFKModel.data.playerInfo[i].piaoNum
				end
			end
		...
		for n = #details, 1, -1 do
			...
			if opType ~= "settle" then
				...
				elseif opType == "gang" then
					content[1] = (details[n].score < 0 and "被" or "" ) .. MjXzFKClearing.GetGangName(op)
					content[2] = MjXzFKClearing.GetGangPlusCount(op)
						...
						for _, seatId in ipairs(details[n].affected) do
							...
							for _, info in ipairs(settleInfo) do
								if info.seat_num == seatId then
									...
									self:CreateDetailItem(content[1], content[2], (details[n].score < 0 and "-" or "+") .. StringHelper.ToCash(math.abs(details[n].getScore)), MjXzFKClearing.GetAffectedPlayerPos(seatno, {seatId}), isBroke)
									break
								end
							end
						end
				...
				if opType == "gang" or (opType == "hu" and details[n].score > 0) or op == "zhuan_yu" then
					self:CreateDetailItem(content[1], content[2], (details[n].score < 0 and "-" or "+") .. StringHelper.ToCash(math.abs(details[n].getScore)), MjXzFKClearing.GetAffectedPlayerPos(seatno, details[n].affected), details[n].score ~= details[n].getScore)
				end
			else
				if op == "cj" then
				...
				for _, d in ipairs(details[n].data) do
					...
					if (details[n].score > 0 and rs < 0) or (details[n].score < 0 and rs > 0) then
						...
						self:CreateDetailItem(content[1], content[2], (rs > 0 and "-" or "+") .. StringHelper.ToCash(math.abs(d.score)), MjXzFKClearing.GetAffectedPlayerPos(seatno, {d.cur_p}), d.lose_surplus ~= 0)
					end
				end
			end
		end
...
function MjXzFKClearing:FormatNum(num)
...
function MjXzFKClearing:game_share()
	self:OnShareClick()
end

function MjXzFKClearing:OnChangeClick()
	DdzZJFChangePrefab.Create(nil,DdzFKModel.data.game_type)
end
