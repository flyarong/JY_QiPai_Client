...
function MjXzFKPlayerManger:ctor(transform, pos,gamePanel)
	...
	if self:IsMyself() then
		self.ShouPai = MjMyShouPaiManger3D.Create(tran, self , MjXzFKLogic , MjXzFKModel , MjXzZJFGamePanel3D)
		...
		self.ShouPai = MjShouPaiManger3D.Create(tran, self , MjXzFKLogic , MjXzFKModel , MjXzZJFGamePanel3D)
	end
...
function MjXzFKPlayerManger:daPiaoFinish()
	self.ShouPai:RotateUpShouPai()
end

function MjXzFKPlayerManger:adjustShouPaiPos()
	if self:IsMyself() then
		--dump(MjXzFKModel.cardScale , "<color=yellow>------------ adjustShouPaiPos:</color>")
...
function MjXzFKPlayerManger:PlayerFapai()
	...
	MjXzZJFGamePanel3D.PengGangMag:Clear(self.uiPos)
end
...
function MjXzFKPlayerManger:Refresh()
	...
	MjXzZJFGamePanel3D.ChupaiMag:Refresh(self.uiPos, self.playerData.cpList, model.cur_chupai)
	MjXzZJFGamePanel3D.PengGangMag:Refresh(self.uiPos, self.playerData.pgList)
	...
	if MjXzFKModel.daPiao and MjXzFKModel.data.status == MjXzFKModel.Status.fp or (MjXzFKModel.data.status == MjXzFKModel.Status.da_piao and playerInfo.piaoNum == -1) then
...
function MjXzFKPlayerManger:BackChupai()
	MjXzZJFGamePanel3D.ChupaiMag:DelTail()
...
function MjXzFKPlayerManger:PlayMusicEffect(val)
	local playerInfo = MjXzFKModel.GetPosToPlayer(self.uiPos)
	dump({playerInfo=playerInfo,uiPos=self.uiPos},"<color=red>播放音效</color>")
	if playerInfo then
		local sound
