...
local function MakeLister()
    lister={}
    lister["friendgame_all_info"] = this.on_friendgame_all_info
    lister["friendgame_join_msg"] = this.on_friendgame_join_msg
    lister["friendgame_quit_msg"] = this.on_friendgame_quit_msg
    lister["friendgame_gameover_msg"] = this.on_friendgame_gameover_msg
    lister["friendgame_begin_game_response"] = this.on_friendgame_begin_game_response
    lister["friendgame_net_quality"] = this.on_friendgame_net_quality
    lister["friendgame_gamecancel_msg"] = this.on_friendgame_gamecancel_msg
    ...
    lister["friendgame_begin_vote_cancel_room_msg"] = this.on_friendgame_begin_vote_cancel_room_msg
    lister["friendgame_over_vote_cancel_room_msg"] = this.on_friendgame_over_vote_cancel_room_msg
    lister["friendgame_player_vote_cancel_room_msg"] = this.on_friendgame_player_vote_cancel_room_msg
...
local function MsgDispatch(proto_name, data)
    ...
    if data.status_no then
        ...
        if proto_name~="nor_ddz_nor_status_info" and  proto_name~="friendgame_all_info" then
...
function DdzFKModel.on_nor_ddz_nor_ready_msg(proto_name, data)
    ...
    if not DdzFKModel.data.ready then
        DdzFKModel.data.ready = {0,0,0,0}
    end
...
function DdzFKModel.on_friendgame_all_info(proto_name, data)
    ...
    if data.status_no==-1 then
        ...
        m_data.friendgame_room_no = data.friendgame_room_no
...
function DdzFKModel.on_nor_ddz_nor_replay_game_response(proto_name,data)
    ...
    if data.result == 0 then
        ...
        HintPanel.Create(1, msg, function ()
            ...
            InitMatchData()
            MainLogic.ExitGame()
            DdzFKLogic.change_panel("DdzFKHallPanel")
        end)
    end
end
