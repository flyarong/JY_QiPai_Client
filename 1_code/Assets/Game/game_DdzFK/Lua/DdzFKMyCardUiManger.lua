...
DdzFKMyCardUiManger = basefunc.class()

local rule={
center="center", --居中
}
--管理规则
DdzFKMyCardUiManger.mangerRule=nil
--node
DdzFKMyCardUiManger.node=nil
--牌列表
DdzFKMyCardUiManger.itemList=nil
--组件的宽度
DdzFKMyCardUiManger.itemWidth=0
DdzFKMyCardUiManger.width=0
--item间隔
DdzFKMyCardUiManger.itemInterval=0
--
DdzFKMyCardUiManger.moveDuration=0.2
local instance
function DdzFKMyCardUiManger.Create(node,itemWidth,itemHeight,itemInterval)
	instance=DdzFKMyCardUiManger.New()
...
function DdzFKMyCardUiManger:MyExit()
...
--[[
	...
	no
--]]
function DdzFKMyCardUiManger:addItemByOrder(item,isRefresh)
...
--按权值添加item
function DdzFKMyCardUiManger:addItemByWeight(item,isRefresh)
	local pos=nil
...
--从容器中移除
function DdzFKMyCardUiManger:RemoveItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			self.cardCount=self.cardCount-1
...
--删除
function DdzFKMyCardUiManger:DeleteItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			value:Destroy()
			self.cardCount=self.cardCount-1
...
--删除all
function DdzFKMyCardUiManger:DeleteAllItem()
	for idx,value in ipairs(self.itemList) do
		value:Destroy()
	end
	self.cardCount=0
...
--计算Item位置
function DdzFKMyCardUiManger:CalculationPos()
	if self.mangerRule==rule.center then
 		self:CalculationPosByCenter()
	end
end
function DdzFKMyCardUiManger:Refresh()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzFKMyCardUiManger:RefreshWithAni()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzFKMyCardUiManger:CalculationPosByCenter()
	local offsetX=0
...
function DdzFKMyCardUiManger:setRect(width,height,offsetX)
...
function DdzFKMyCardUiManger:clearItemChooseStatus()
end

function DdzFKMyCardUiManger:chooseIetm(pos)
...
function DdzFKMyCardUiManger:GetUIPos(pos)
...
--智能补全
function DdzFKMyCardUiManger:intelligent_completion(list,frist_tanqi)
	local data=DdzFKModel.data
	if list and #list>0 and #list<10 and data and data.status==DdzFKModel.Status.cp and data.cur_p==data.seat_num then
		local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(data.action_list)
		local type=0
		local pai=nil
		if pos then
			DdzFKModel.data.ddz_algorithm:get_cpInfo_by_action(data.action_list[pos])
			type=data.action_list[pos].type
			pai=data.action_list[pos].pai
		end

		local ic_map=DdzFKModel.data.ddz_algorithm:intelligent_completion(type,pai,list,data.my_pai_list,data.laizi,frist_tanqi)
		--用于标记已经弹出
		local flag={}
		if ic_map then
			local _pai
...
function DdzFKMyCardUiManger:update()
