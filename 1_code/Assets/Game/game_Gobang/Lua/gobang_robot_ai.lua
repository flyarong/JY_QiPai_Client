...
function gobang_robot_ai.getGrade(situation)
    ...
    if situation.alive4 >= 1 or situation.die4 >= 2 or
        (situation.die4 >= 1 and situation.alive3 >= 1) then
...
function gobang_robot_ai.Level1()
    local x = 1
    local y = 1
    MathExtend.SortListCom(attackGradeList, CompareCall)
    MathExtend.SortListCom(defenseGradeList, CompareCall)
    --优先攻击
    if attackGradeList[1].grade > LevelEight and defenseGradeList[1].grade <= Levelseven then--攻击低级活3即以上
