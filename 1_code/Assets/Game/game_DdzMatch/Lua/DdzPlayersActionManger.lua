...
local instance
DdzPlayersActionManger = basefunc.class()
function DdzPlayersActionManger.Create(gamePanel)
	instance=DdzPlayersActionManger.New()
	--no 2 object
	...
	instance.uiManager=DdzMyCardUiManger.Create(instance.gamePanel.playerself_operate_son.cards_remain,212,302,90)
	instance.cardObj = GetPrefab("DdzCard")
	return instance
end
function DdzPlayersActionManger:Refresh()
	local hash={}
	local flag=false
	--
	...
	if DdzMatchModel.data then
		my_pai_list=DdzMatchModel.data.my_pai_list
	end 
	if my_pai_list then
...
--添加牌
function DdzPlayersActionManger:AddPai(pai_list)
...
function DdzPlayersActionManger:Fapai(pai_list)
...
--加倍回调
function DdzPlayersActionManger:JiabeiBtnCB()
...
--不加倍回调
function DdzPlayersActionManger:BujiabeiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--jdz 1 回调
function DdzPlayersActionManger:Jdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--jdz 2 回调
function DdzPlayersActionManger:Jdz2BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--jdz 3 回调
function DdzPlayersActionManger:Jdz3BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--不叫地主
function DdzPlayersActionManger:Bujdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
-- 抢地主
function DdzPlayersActionManger:Qdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--不抢地主
function DdzPlayersActionManger:Buqdz1BtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
...
--出牌回调
function DdzPlayersActionManger:ChupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
	if m_data then
		--情况自动出最后一手牌回调
		...
		local ret, list_table = DdzMatchModel.ddz_algorithm:check_chupai_safe(m_data.action_list, pai_list, m_data.laizi, DdzMatchModel.data.remain_pai_amount[DdzMatchModel.data.seat_num])
		if ret then
			...
			if index == 1 then
				...
				local gamePanel = DdzMatchLogic.get_cur_panel()
				if gamePanel then
...
function DdzPlayersActionManger:SendChupaiRequest(act)
	local m_data=DdzMatchModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	if m_data and Network.SendRequest("nor_ddz_nor_chupai", act) then
		if m_data.countdown and m_data.countdown>0 then
			act.myrate=DdzMatchModel.data.my_rate
			self:DealAction(1,act)
		end
	else
		DDZAnimation.Hint(2,Vector3.New(0,-350,0), Vector3.New(0,0,0))
...
--不出牌和要不起回调
function DdzPlayersActionManger:ChangeCardsPosBtnCB()
	--将牌复位牌
	for no,v in pairs(self.my_pai_hash) do
		v:ChangePosStatus(0)
	end
...
--不出牌和要不起回调
function DdzPlayersActionManger:BuchupaiBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	local m_data=DdzMatchModel.data
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
...
--提示回调
function DdzPlayersActionManger:HintBtnCB()
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	--情况自动出最后一手牌回调
	self.gamePanel.last_pai_auto_cb=nil
	...
	local get_hint_by_act=function ()
		--选取
		local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(DdzMatchModel.data.action_list)
		local type=nil
		local pai=nil
		if pos then
			type=DdzMatchModel.data.action_list[pos].type
			pai=DdzMatchModel.data.action_list[pos].pai
		end
		self.my_hint_chupai=DdzMatchModel.ddz_algorithm:cp_hint(type,pai, DdzMatchModel.data.my_pai_list,DdzMatchModel.data.laizi)
	end

	if not self.my_hint_chupai or self.my_hint_chupai.type==0  then
		get_hint_by_act()
	else
		self.my_hint_chupai=DdzMatchModel.ddz_algorithm:cp_hint(self.my_hint_chupai.type,self.my_hint_chupai.pai, DdzMatchModel.data.my_pai_list,DdzMatchModel.data.laizi)
	end
	--循环
	if not self.my_hint_chupai or self.my_hint_chupai.type==0 then
		get_hint_by_act()
...
function DdzPlayersActionManger:DealAction(seatNum,action)
	if seatNum==1 then
		if action.type<100 and action.type>0 then
			DdzMatchModel.ddz_algorithm:get_cpInfo_by_action(action)
		end
...
function DdzPlayersActionManger:CheckActionIsRun(action)
	...
	--从服务器发过来的
		...
		if not self.my_last_action then
			...
			if checkActionIsEquel(action,self.my_last_action) then
				...
				--刷新我的牌列表
				self:Refresh()
				self.gamePanel.DdzActionUiManger:RefreshActionWithAni(1,action)
...
--出牌action
function DdzPlayersActionManger:ChupaiAction(seatNum,action)
	...
	--提交给actionUimanger
	self.gamePanel.DdzActionUiManger:RefreshActionWithAni(seatNum,action)

	--刷新warning和剩余的pai
	self.gamePanel:RefreshRemainPaiWarningStatusWithAni(seatNum,action.type,remain_card) 
end
--其他操作
function DdzPlayersActionManger:OtherAction(seatNum,action)
	--如果是我自己的操作
	if seatNum==1 then
...
function DdzPlayersActionManger:MyExit()
	if instance then
...
function DdzPlayersActionManger:ChangeClickStatus(status)
