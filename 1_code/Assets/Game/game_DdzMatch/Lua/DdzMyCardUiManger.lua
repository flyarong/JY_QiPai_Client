...
DdzMyCardUiManger = basefunc.class()

local rule={
center="center", --居中
}
--管理规则
DdzMyCardUiManger.mangerRule=nil
--node
DdzMyCardUiManger.node=nil
--牌列表
DdzMyCardUiManger.itemList=nil
--组件的宽度
DdzMyCardUiManger.itemWidth=0
DdzMyCardUiManger.width=0
--item间隔
DdzMyCardUiManger.itemInterval=0
--
DdzMyCardUiManger.moveDuration=0.2

local instance
function DdzMyCardUiManger.Create(node,itemWidth,itemHeight,itemInterval)
	instance=DdzMyCardUiManger.New()
...
function DdzMyCardUiManger:MyExit()
...
--[[
	...
	no
--]]
function DdzMyCardUiManger:addItemByOrder(item,isRefresh)
...
--按权值添加item
function DdzMyCardUiManger:addItemByWeight(item,isRefresh)
	local pos=nil
...
--从容器中移除
function DdzMyCardUiManger:RemoveItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			self.cardCount=self.cardCount-1
...
--删除
function DdzMyCardUiManger:DeleteItemByNo(no,isRefresh)
	for idx,value in ipairs(self.itemList) do
		if value.no==no then
			value:Destroy()
			self.cardCount=self.cardCount-1
...
--删除all
function DdzMyCardUiManger:DeleteAllItem()
	for idx,value in ipairs(self.itemList) do
		value:Destroy()
	end
	self.cardCount=0
...
--计算Item位置
function DdzMyCardUiManger:CalculationPos()
	if self.mangerRule==rule.center then
 		self:CalculationPosByCenter()
	end
end
function DdzMyCardUiManger:Refresh()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
	...
	if DdzMatchModel.data then
		if DdzMatchModel.data.dizhu then
...
function DdzMyCardUiManger:RefreshWithAni()
	self:CalculationPos()
	for idx,value in ipairs(self.itemList) do
...
function DdzMyCardUiManger:CalculationPosByCenter()
	local offsetX=0
...
function DdzMyCardUiManger:setRect(width,height,offsetX)
...
function DdzMyCardUiManger:clearItemChooseStatus()
end

function DdzMyCardUiManger:chooseIetm(pos)
...
--智能补全
function DdzMyCardUiManger:intelligent_completion(list,frist_tanqi)
	local data=DdzMatchModel.data
	if list and #list>0 and #list<10 and data and data.status==DdzMatchModel.Status.cp and data.cur_p==data.seat_num then
		local pos=nor_ddz_base_lib.get_real_chupai_pos_by_act(data.action_list)
		local type=0
		local pai=nil
		if pos then
			DdzMatchModel.ddz_algorithm:get_cpInfo_by_action(data.action_list[pos])
			type=data.action_list[pos].type
			pai=data.action_list[pos].pai
		end

		local ic_map=DdzMatchModel.ddz_algorithm:intelligent_completion(type,pai,list,data.my_pai_list,data.laizi,frist_tanqi)
		--用于标记已经弹出
		local flag={}
		if ic_map then
			local _pai
...
function DdzMyCardUiManger:update()
