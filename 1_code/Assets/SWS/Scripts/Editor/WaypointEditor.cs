...
namespace SWS
{
    ...
    public class WaypointEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            //draw path text label
            GUILayout.Label("Enter Waypoint Manager Name: ", GUILayout.Height(15));
            //display text field for creating a path with that name
            wm_name = EditorGUILayout.TextField(wm_name, GUILayout.Height(15));
            //draw path creation button
            if (!placing && GUILayout.Button("Exprot Json Path", GUILayout.Height(40)))
            {
                ExportJson(wm_name);
            }
            //draw path creation button
            if (!placing && GUILayout.Button("Exprot Lua Path", GUILayout.Height(40)))
            {
                ExportLua(wm_name);
            }
        }
        ...
        Dictionary<string,Vector2[]> Getpaths(string str)
        {
            Dictionary<string,Vector2[]> paths = new Dictionary<string, Vector2[]>();
            var wm = GameObject.Find("Waypoint Manager" + str);
            var pat = wm.GetComponentsInChildren<PathManager>();
            for (int i = 0; i < pat.Length; i++)
            {
                var item = pat[i];
                int len = item.waypoints.Length;
                Vector2[] pos_arr = new Vector2[len];
                for (int k = 0; k < len; k++)
                {
                    var pos = item.waypoints[k].position;
                    pos_arr[k] = pos;
                }
                paths.Add(i.ToString(),pos_arr);
            }
            return paths;
        }

        void ExportJson (string str){
            Debug.Log("导出json数据");
            var paths = Getpaths(str); 
            ...
            Debug.Log("json:" + json);
            string path =  Application.dataPath + @"/SWS/JsonPath" + wm_name + ".lua";	//这里保存文件的路径
        ...
        Dictionary<string,  Dictionary<string, object>> GetDicWayPoints(string str){
            Dictionary<string,  Dictionary<string, object>> paths = new Dictionary<string,  Dictionary<string, object>>();
            var wm = GameObject.Find("Waypoint Manager" + str);
            var pm_arr = wm.GetComponentsInChildren<PathManager>();
            for (int i = 0; i < pm_arr.Length; i++)
            {
                ...
                for (int k = 0; k < len; k++)
                {
                    var _pos = item.waypoints[k].position;
                ...
                Dictionary<string, object> path_dic = new Dictionary<string, object>();
                path_dic.Add("WayPoints",wayPoints);
                path_dic.Add("id",(i + 1));
                path_dic.Add("type",1);
                builder.Append ("]]");
                path_dic.Add("builder",builder.ToString());
                paths.Add((i + 1).ToString(),path_dic);
            }
            return paths;
        }

        void ExportLua (string str){
            var paths = GetDicWayPoints(str);// GetDicpaths(); 
            var lua = TileEditor.HELua.Encode(paths);//TileEditor.HELua.Encode(paths);
            Debug.Log(lua);
            string path =  Application.dataPath + @"/SWS/LuaPath" + wm_name + ".lua";	//这里保存文件的路径
            StringBuilder builder = new StringBuilder();
            builder.Append("return ");
            builder.Append(lua);
            // builder.Append("'");
