...
class ChannelProcessBuilderWUZIQI_VIVO : IPreprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
		string channelName = AppDefine.CurQuDao;
		if (string.Compare (channelName, "vivo_wuziqi", true) != 0)
			return;

		UnityEngine.Debug.Log ("OnPreprocessBuild for vivo_wuziqi channel!");

		PlayerSettings.applicationIdentifier = "com.gaoshouhuyu.wuziqi.vivo";
...
class PostProcessBuilderWUZIQI_VIVO : IPostprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPostprocessBuild(BuildTarget buildTarget, string path) {
		...
		UnityEngine.Debug.Log ("OnPostprocessBuild for vivo_wuziqi channel!");
	...
	private void PostprocessAndroid(string path) {
		...
		if (!string.IsNullOrEmpty(Packager.ChannelConfig.channel) && Packager.ChannelConfig.channel != "wuziqi_vivo")
        {
