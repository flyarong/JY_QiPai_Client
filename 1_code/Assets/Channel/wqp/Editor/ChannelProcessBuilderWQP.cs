...
class ChannelProcessBuilderWQP : IPreprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
		string channelName = AppDefine.CurQuDao;
		if (string.Compare (channelName, "wqp", true) != 0)
			return;

		UnityEngine.Debug.Log ("OnPreprocessBuild for wqp channel!");

		PlayerSettings.applicationIdentifier = "com.gaoshou.wqpddz";
...
class PostProcessBuilderWQP : IPostprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPostprocessBuild(BuildTarget buildTarget, string path) {
		...
		UnityEngine.Debug.Log ("OnPostprocessBuild for wqp channel!");
	...
	private void PostprocessAndroid(string path) {
		...
		if (!string.IsNullOrEmpty(Packager.ChannelConfig.channel) && Packager.ChannelConfig.channel != "wqp")
        {
