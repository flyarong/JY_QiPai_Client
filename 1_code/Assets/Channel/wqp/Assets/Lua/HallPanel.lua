...
function HallPanel:SetupBtns()
	...
	self.PEBox.PointerClick:AddListener(function (obj)
		...
		if MainModel.UserInfo.xsyd_status == 1 then
			...
			GameFreeModel.RapidBeginGameLevel(PlayerPrefs.GetInt(MainModel.FreeRapidBeginKey, 1), obj.transform.parent.transform)
		end
	end)
	...
	self.MINIGAMEBox1.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		self:OnMiniGameClick(obj)
	end)

	self.QYSBox.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		self:OnMatchClick(obj)
	end)
	self.QYSBox1.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	...
	self.JJBYBox.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		GameManager.GotoUI({gotoui = "game_FishingHall",goto_scene_parm = {down_style={panel=self.JJBYBox.transform}}})
	end)
	self.JJBYBox1.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		GameManager.GotoUI({gotoui = "game_FishingHall",goto_scene_parm = {down_style={panel=self.JJBY1.transform}}})
	end)

	self.XXLBox1.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		GameManager.GotoUI({gotoui="game_Eliminate", down_style={panel=self.XXL1.transform}})
	end)

	self.SHXXLBox.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		GameManager.GotoUI({gotoui="game_EliminateSH", down_style={panel=self.SHXXL.transform}})
	end)
end
...
function HallPanel:ctor(call)
	...
	self.QYSBox = self.QYSBox:GetComponent("PolygonClick")
	self.PEBox = self.PEBox:GetComponent("PolygonClick")

	self.JJBYBox1 = self.JJBYBox1:GetComponent("PolygonClick")
	self.MINIGAMEBox1 = self.MINIGAMEBox1:GetComponent("PolygonClick")
	self.QYSBox1 = self.QYSBox1:GetComponent("PolygonClick")
	self.XXLBox1 = self.XXLBox1:GetComponent("PolygonClick")
	...
	RedHintManager.UpdateRedHint(RedHintManager.RedHintKey.RHK_XYCJ)
	 --年末回馈修改UI
	--Event.Brocast("act_match_order_msg_change",{ButtonGet_Game = self.activity_lmp:GetComponent("Image"),wqp_hall_img = self.QYS1.gameObject.transform:Find("Image"):GetComponent("Image")})
	...
	btn_map["right_top"] = {self.hall_btn_5_2,self.hall_btn_5_1,self.hall_btn_root1,self.hall_btn_4, self.hall_btn_3, self.hall_btn_2, self.hall_btn_1}
	btn_map["top"] = {self.hall_btn_top}
	btn_map["top1"] = {self.hall_btn_top1}
	btn_map["l_border"] = {self.hall_btn_l_border}
	self.game_btn_pre = GameButtonPanel.Create(btn_map, "hall_config", self.transform)
	self:MyRefresh()
	--年末回馈修改UI
	Event.Brocast("act_match_order_msg_change",{hall_img = self.QYS1.gameObject.transform:Find("Image"):GetComponent("Image")})

	if MainModel.IsHWLowPlayer() then
		self.right1.gameObject:SetActive(true)
		self.right2.gameObject:SetActive(false)
	else
		self.right1.gameObject:SetActive(false)
		self.right2.gameObject:SetActive(true)
	end
	...
	local temp_ui = {}
	LuaHelper.GeneratingVar(self.MiniHongbao.transform,temp_ui)
	local ca = CommonCellAnim.Create()
	ca:Go(temp_ui.move_node,temp_ui.hongbao,2,4,1)
	self.MiniHongbao.gameObject:SetActive(false)
	...
	self.JBS.transform:GetComponent("Image").sprite = GetTexture("hall_btn_2")
	self.QYS1.transform:GetComponent("Image").sprite = GetTexture("hall_btn_1")
	self.QYS1.transform:Find("Image").transform:GetComponent("Image").sprite = GetTexture("hall_btn_1_3")
	self.JJBY1.transform:GetComponent("Image").sprite = GetTexture("jjby_bg_1")
	self.JJBY1.transform:Find("tit_by").transform:GetComponent("Image").sprite = GetTexture("jjby_word_1")
	self.JJBY1.transform:Find("fish").transform:GetComponent("Image").sprite = GetTexture("img_yu")
	self.JJBY1.transform:Find("gun").transform:GetComponent("Image").sprite = GetTexture("img_pao")
	self.JJBY1.transform:Find("caidai").transform:GetComponent("Image").sprite = GetTexture("hall_btn_4_1")
	self.JJBY1.transform:Find("Image (5)").transform:GetComponent("Image").sprite = GetTexture("hall_btn_4_4")
	self.MINIGAME1.transform:GetComponent("Image").sprite = GetTexture("xyx_bg_1")
	self.MINIGAME1.transform:Find("xyx_tit").transform:GetComponent("Image").sprite = GetTexture("xyx_word_1")
	self.MINIGAME1.transform:Find("sz").transform:GetComponent("Image").sprite = GetTexture("hall_img_sz")
	self.MINIGAME1.transform:Find("mf").transform:GetComponent("Image").sprite = GetTexture("img_mf")
	self.PE.transform:GetComponent("Image").sprite = GetTexture("hall_btn_3")
...
function HallPanel:RefreshQYSMatch()
	...
	local pre0 = self.jiangbei.transform:GetChild(0)
	local pre1 = self.jiangbei.transform:GetChild(1)
	if b then
		pre0.gameObject:SetActive(true)
		pre1.gameObject:SetActive(true)
		self.XXL1.gameObject:SetActive(false)
	else
		pre0.gameObject:SetActive(false)
		pre1.gameObject:SetActive(false)
		self.XXL1.gameObject:SetActive(true)
	end
end
...
function HallPanel:OnMiniGameClick(go)
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	GameManager.GotoUI({gotoui = "game_MiniGame",goto_scene_parm = {down_style={panel=self.MINIGAME1.transform}}})
end
...
function HallPanel:SetQMZQ()
	...
	-- end
	-- self.transform:Find("DownL/@RectDownL/BtnRT/@hall_btn_9/@money_center_btn/money_center_btn/hall_btn_sc/@gain_141").gameObject:SetActive(not self.is_opened_time_over_a_day)
	--将鲸鱼斗地主和玩棋牌中的全返礼包和赚钱的tips提示改为红点提示。
