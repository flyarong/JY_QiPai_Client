...
class PreProcessBuilderCaiYunMJ_YYB_main : IPreprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
		string channelName = AppDefine.CurQuDao;
		if (string.Compare (channelName, "caiyunmj_yyb_main", true) != 0)
			return;

		UnityEngine.Debug.Log ("OnPreprocessBuild for caiyunmj_yyb_main channel!");

		PlayerSettings.applicationIdentifier = "com.tencent.tmgp.caiyunmj";
	...
	private void CopyPluginDir(string rootDir, string tagName) {
		...
		Debug.Log (rootDir + "Channel/" + AppDefine.CurQuDao + pluginDir);
		Debug.Log (Application.dataPath + pluginDir);
	}
}

class PostProcessBuilderCaiYunMJ_YYB_main : IPostprocessBuild {
	public int callbackOrder { get { return 0; } }
	public void OnPostprocessBuild(BuildTarget buildTarget, string path) {
		...
		UnityEngine.Debug.Log ("OnPostprocessBuild for caiyunmj_yyb_main channel!");
