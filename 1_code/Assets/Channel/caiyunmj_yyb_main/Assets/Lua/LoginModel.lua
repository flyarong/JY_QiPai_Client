...
function LoginModel.ChannelLuaTableToJson(channel, kvs, mergeTbl)
    if not this or not this.login_LTD or not this.login_LTD[channel] then
        return ""
    end
...
function LoginModel.GetChannelLuaTable(channel)
    if not this or not this.login_LTD then
        return nil
    end
...
local function InitLoginData()
    ...
    dump(this.login_LTD, "<color=red>EEE this.login_LTD</color>")
