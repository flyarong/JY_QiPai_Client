...
function LoginLogic.OnLoginResult(result)
    ...
    elseif result == 2153 
        or result == 2155 
        or result == 2156 
        or (result == 2150 and curChannel=="huawei") then

        if wechatErrorStatus == 0 then
            cancelLogin()
            LoginModel.ClearChannelData("huawei")
            wechatErrorStatus = 1
...
function LoginLogic.WechatLogin()
    ...
    readLogin("huawei",loginId,refresh_token)
end
...
function LoginLogic.AutoLogin()
	...
	elseif loginModel.loginData.lastLoginChannel == "huawei" then
