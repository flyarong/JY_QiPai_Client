...
class PreProcessBuilderHuanLeBY_VIVO_NOUPDATE : IPreprocessBuild {
	...
	public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
		...
		if (string.Compare (channelName, "vivo_cymj", true) != 0)
			return;

		UnityEngine.Debug.Log ("OnPreprocessBuild for vivo_cymj channel!");

		PlayerSettings.applicationIdentifier = "com.gaoshouhuyu.cymj.vivo";
...
class PostProcessBuilderHuanLeBY_VIVO_NOUPDATE : IPostprocessBuild {
	...
	public void OnPostprocessBuild(BuildTarget buildTarget, string path) {
		...
		UnityEngine.Debug.Log ("OnPostprocessBuild for vivo_cymj channel!");
	...
	private void PostprocessAndroid(string path) {
		...
		if (!string.IsNullOrEmpty(Packager.ChannelConfig.channel) && Packager.ChannelConfig.channel != "vivo")
        {
