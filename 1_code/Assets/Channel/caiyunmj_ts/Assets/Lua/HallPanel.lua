...
function HallPanel:SetupBtns()
	...
	self.GYSBox.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		self:OnMatchQYSClick(obj)
	end)
	self.MJBox.PointerClick:AddListener(function (obj)
		ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
		GameFreeModel.RapidBeginGameLevel(17, obj.transform.parent.transform)
	end)
end
...
function HallPanel:ctor(call)
	...
	self.GYSBox = self.GYSBox:GetComponent("PolygonClick")
	self.MJBox = self.MJBox:GetComponent("PolygonClick")
...
function HallPanel:OnOff()
	...
	self.VIP_btn.gameObject:SetActive(false)
end
...
function HallPanel:OnMatchQYSClick(go)
	...
	local goto_scene_parm = {hall_type = MatchModel.HallType.fls, down_style={panel=go.transform.parent.transform}}
	--年末回馈修改UI
...
function HallPanel:OnServiceClick(go)
	ExtendSoundManager.PlaySound(audio_config.game.com_but_confirm.audio_name)
	if not IsEquals(self.ServiceNode) then return end
...
function HallPanel:set_vip_info()
	...
	if not GameGlobalOnOff.Vip or PlayerPrefs.GetInt(VIPManager.get_vip_level().."_hall_vip_click",0) == 1 then
