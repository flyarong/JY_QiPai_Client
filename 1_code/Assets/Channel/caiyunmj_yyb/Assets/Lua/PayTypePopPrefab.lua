...
function PayTypePopPrefab.Create(goodsid, desc, createcall,convert)
    ...
    dump(request, "<color=green>midas_create_pay_order</color>")
	Network.SendRequest("midas_create_pay_order", request, function(_data)
		dump(_data, "<color=green>midas_create_pay_order_response</color>")
		if _data.result == 0 then
			local luaData = {
				ext = "recharge",
				img = imgFile,

				ident = _data.order_id,
				url = _data.url_params
			}
			sdkMgr:Pay(lua2json(luaData), function(json_data)
				local lua_tbl = json2lua(json_data)
				if not lua_tbl then
					print("[PAY] pay exception: json_data invalid")
					return
				end

				dump(lua_tbl, "[PAY] result")

				if lua_tbl.result == 0 and lua_tbl.errno == 0 then
					if createcall then createcall(0) end
				else
					local error_tbl = {
						[1] = 2248,
						[3100] = 2246,
					...
					if error_tbl[lua_tbl.errno] then
						HintPanel.ErrorMsg(error_tbl[lua_tbl.errno])
					else
						local channel = MainModel.LoginInfo.channel_type or ""
						HintPanel.Create(1, "支付异常(" .. channel .. ":" .. lua_tbl.errno .. ")")
					end
				end
			end)
		else
