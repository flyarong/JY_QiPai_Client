...
package.loaded["Game.caiyunmj_yyb.Lua.TLOG"] = nil
require "Game.caiyunmj_yyb.Lua.TLOG"
...
local function wechatTokenToLogin()
    ...
    --[[sdkMgr:Login("yyb_wechat",
        function(token)
            if token then
	    	local segs = basefunc.string.split(token, ";")
		if segs == nil or #segs ~= 6 then
			print("[WX Login] login2json failed:" .. token)
			return
		end
		...
		print("[logininginging] wx loging:" .. curToken)

		local tbl = {
			openid = segs[1],
			token = segs[2],
			refresh_token = segs[3],
			pf = segs[4],
			pfkey = segs[5],
			paytoken = segs[2],
			stamp = os.time()
		}
		loginModel.UpdateChannelLuaTable("yyb_wechat", tbl)
		...
		print("compare json_data:", json_data1, json_data)
                ...
                local loginData = {
                    ...
                    device_id = loginModel.loginData.device_id,
                    device_os = loginModel.loginData.device_os,
		    market_channel = loginModel.loginData.market_channel,
		    market_platform = loginModel.loginData.market_platform
                }
		Network.SendRequest("login", loginData)
                print("wx token login "..json_data.." curToken:" .. curToken)
            end
        end,
	function(result)
		FullSceneJH.RemoveAll()
		status = nil
		HandleLoginFailedResult(result)
        cancelLogin()
	end
    )]] print(
        "wechatTokenToLogin"
...
local function qqTokenToLogin()
    ...
    --[[sdkMgr:Login("yyb_qq",
        function(token)
	        ...
	        local segs = basefunc.string.split(token, ";")
		if segs == nil or #segs ~= 6 then
			print("[QQ Login] login2json failed:" .. token)
			return
		end
		...
		print("[logininginging] qq loging:" .. curToken)

		local tbl = {
			openid = segs[1],
			token = segs[2],
			refresh_token = segs[2],
			pf = segs[4],
			pfkey = segs[5],
			paytoken = segs[6],
			stamp = os.time()
		}
		loginModel.UpdateChannelLuaTable("yyb_qq", tbl)
		...
		Network.SendRequest("login", loginData)
                print("qq token login "..json_data .. " raw: " .. token)
            end
        end,
	function(result)
		FullSceneJH.RemoveAll()
